...
func promptContent(cwd string, blocks []acp.ContentBlock) (string, []string, error) {
	...
	for _, block := range blocks {
		...
		case block.ResourceLink != nil:
			...
			u, err := url.Parse(uri)
			if err != nil || u.Scheme != "file" || (u.Host != "" && u.Host != "localhost") || u.User != nil || u.RawQuery != "" || u.Fragment != "" {
				return "", nil, fmt.Errorf("resources must be local absolute file URIs without query or fragment")
			}
			native := nativePathFromFileURL(u)
			if !filepath.IsAbs(native) {
				return "", nil, fmt.Errorf("resources must be local absolute file URIs without query or fragment")
			}
			if base := filepath.Base(native); base != strings.TrimSpace(base) {
				return "", nil, fmt.Errorf("resource path has leading or trailing whitespace")
			}
			location := findingLocation(cwd, contract.Comment{Path: native})
			if location == nil {
