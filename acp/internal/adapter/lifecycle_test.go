...
func TestSessionAcceptsPromptAfterCancellation(t *testing.T) {
	ready := make(chan struct{})
	a := NewAgent("ocr", fakeRunner{}, intent.NewParser(waitingParser{ready}, nil))
	...
	go func() {
		response, err := a.Prompt(ctx, acp.PromptRequest{SessionId: s.SessionId, Prompt: []acp.ContentBlock{acp.TextBlock("review please")}})
		if err == nil && response.StopReason != acp.StopReasonCancelled {
			err = errors.New("first prompt did not return cancelled")
		}
		done <- err
	}()
	select {
	...
	if err := a.Cancel(ctx, acp.CancelNotification{SessionId: s.SessionId}); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-done:
	...
	response, err := a.Prompt(ctx, acp.PromptRequest{SessionId: s.SessionId, Prompt: []acp.ContentBlock{acp.TextBlock("/review")}})
	if err != nil || response.StopReason != acp.StopReasonEndTurn {
		t.Fatalf("next prompt: response=%+v error=%v", response, err)
	}
}
