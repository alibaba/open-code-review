...
func TestStderrBufferChunksDoNotImplyTruncation(t *testing.T) {
	for _, tc := range []struct {
		name      string
		size      int
		truncated bool
	}{
		{"within limit", 48 << 10, false},
		{"over limit", 70000, true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			message := strings.Repeat("x", tc.size)
			var events []Event
			tail, cut, warnings, err := readStderr(strings.NewReader(message+"\n"), 1<<20, 64<<10, func(e Event) { events = append(events, e) })
			if err != nil || cut || tail != message+"\n" {
				t.Fatalf("tail length=%d cut=%v err=%v", len(tail), cut, err)
			}
			wantEvents, wantWarnings := 1, 0
			if tc.truncated {
				wantEvents, wantWarnings = 2, 1
			}
			if len(events) != wantEvents || len(warnings) != wantWarnings {
				t.Fatalf("events=%d warnings=%d", len(events), len(warnings))
			}
			if events[0].Message != message[:min(len(message), 64<<10)] || events[0].Truncated != tc.truncated {
				t.Fatalf("message length=%d truncated=%v", len(events[0].Message), events[0].Truncated)
			}
		})
	}
}

type failingStderrReader struct{ err error }

func (r failingStderrReader) Read(p []byte) (int, error) {
	return copy(p, "[ocr] partial activity"), r.err
}

func TestStderrEmitsPendingLineBeforeReadError(t *testing.T) {
	failure := errors.New("stderr read failed")
	var events []Event
	tail, cut, warnings, err := readStderr(failingStderrReader{failure}, 1024, 1024, func(e Event) { events = append(events, e) })
	if !errors.Is(err, failure) || cut || len(warnings) != 0 || tail != "[ocr] partial activity" {
		t.Fatalf("tail=%q cut=%v warnings=%v err=%v", tail, cut, warnings, err)
	}
	if len(events) != 1 || events[0].Kind != EventProgress || events[0].Message != tail || events[0].Truncated {
		t.Fatalf("pending line missing or changed: %+v", events)
	}
}
...
func TestKillProcessGroup(t *testing.T) {
	...
	if err := killProcessGroup(cmd); err != nil && !strings.Contains(err.Error(), "no such process") && !errors.Is(err, os.ErrProcessDone) && !strings.Contains(err.Error(), "process already finished") && !strings.Contains(err.Error(), "Access is denied") && !strings.Contains(err.Error(), "invalid argument") {
