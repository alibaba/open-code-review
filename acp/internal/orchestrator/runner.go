...
func (r *ProcessRunner) run(ctx context.Context, request Request, limits Limits, events chan Event, outcomes chan<- Outcome) {
	...
	emit := func(event Event) {
		...
		default:
			...
			// Preserve warnings by evicting an older event.
			// Outcome.Warnings remains the authoritative fallback.
			select {
			case <-events:
			default:
			}
...
func (r *ProcessRunner) stop(cmd *exec.Cmd, waited <-chan error) (error, error) {
	// Interrupt gives OCR a chance to emit its cancellation document before the
	// bounded grace period expires. Unix sends SIGINT to the process group;
	// Windows sends CTRL_BREAK to the CREATE_NEW_PROCESS_GROUP leader.
