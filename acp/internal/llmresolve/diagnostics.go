	...
	if u, err := url.Parse(c.ep.URL); err == nil {
		if u.User != nil {
			...
			if password == "" {
				...
				secrets = append(secrets, username, url.User(username).String())
			}
			// Preserve the original userinfo spelling, including lowercase percent
			// escapes, when a provider echoes the configured URL verbatim.
			_, authority, _ := strings.Cut(c.ep.URL, "://")
			if end := strings.IndexAny(authority, "/?#"); end >= 0 {
				authority = authority[:end]
			}
			if at := strings.LastIndexByte(authority, '@'); at >= 0 {
				secrets = append(secrets, authority[:at])
			}
		}
