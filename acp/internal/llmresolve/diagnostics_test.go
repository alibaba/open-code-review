...
func TestDiagnosticsRedactUsernameCredentials(t *testing.T) {
	for _, tc := range []struct{ userinfo, decoded string }{
		{"username-secret", "username-secret"},
		{"username-secret:", "username-secret"},
		{"username%2fsecret", "username/secret"},
	} {
		for _, scheme := range []string{"https", "HTTPS"} {
			endpoint := scheme + "://" + tc.userinfo + "@gateway/v1"
			client := &Client{ep: Endpoint{URL: endpoint, Token: "different-secret"}}
			got := client.diagnosticText("Echo " + endpoint + " " + tc.decoded)
			if strings.Contains(got, tc.userinfo) || strings.Contains(got, tc.decoded) {
