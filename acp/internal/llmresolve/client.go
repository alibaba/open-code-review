...
func anthropicMessagesURL(rawURL string) string {
	return endpointURL(rawURL, "/v1/messages")
}

func endpointURL(rawURL, suffix string) string {
	u, err := url.Parse(rawURL)
	if err != nil {
		// Let request construction report the invalid URL using safe diagnostics.
		return rawURL
	}
