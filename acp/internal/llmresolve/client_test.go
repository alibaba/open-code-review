...
func TestCallToolPreservesEndpointQueryAndEscapedPath(t *testing.T) {
	for _, protocol := range []string{ProtocolOpenAI, ProtocolAnthropic} {
		for _, fullPath := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/full=%v", protocol, fullPath), func(t *testing.T) {
				suffix := "/chat/completions"
				if protocol == ProtocolAnthropic {
					suffix = "/v1/messages"
				}
				const query = "api-version=2024-02-01&key=a%2fb&key=c+d"
				srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					if r.URL.EscapedPath() != "/gateway%2Ftenant"+suffix || r.URL.RawQuery != query {
						t.Errorf("request URI = %q", r.RequestURI)
					}
					if protocol == ProtocolOpenAI {
						_, _ = io.WriteString(w, `{"choices":[{"message":{"tool_calls":[{"function":{"name":"submit_intent","arguments":"{}"}}]}}]}`)
					} else {
						_, _ = io.WriteString(w, `{"content":[{"type":"tool_use","name":"submit_intent","input":{}}]}`)
					}
				}))
				defer srv.Close()
				base := srv.URL + "/gateway%2Ftenant"
				if fullPath {
					base += suffix
				}
				client, err := NewClient(Endpoint{URL: base + "/?" + query + "#fragment", Token: "key", Model: "m", Protocol: protocol})
				if err != nil {
					t.Fatal(err)
				}
				if _, err := client.CallTool(context.Background(), testRequest()); err != nil {
					t.Fatal(err)
				}
			})
		}
