...
func TestExtraEnumCaseCompatibility(t *testing.T) {
	for _, extra := range [][]string{{"--effort", "High"}, {"--effort=MEDIUM"}} {
		args, err := BuildReviewArgs(&ReviewIntent{Type: ReviewTypeWorkspace, Extra: extra})
		if err != nil {
			t.Errorf("review %q: %v", extra, err)
			continue
		}
		if !reflect.DeepEqual(args[1:1+len(extra)], extra) {
			t.Errorf("forwarded flags changed: %q", args)
		}
	}
	for _, extra := range [][]string{{"--batch", "By-Language"}, {"--batch=BY-DIRECTORY"}} {
		args, err := BuildScanArgs(&ScanIntent{Extra: extra})
		if err != nil {
			t.Errorf("scan %q: %v", extra, err)
			continue
		}
		if !reflect.DeepEqual(args[1:1+len(extra)], extra) {
			t.Errorf("forwarded flags changed: %q", args)
...
func TestScanRejectsWhitespacePaddedPaths(t *testing.T) {
	for _, path := range []string{"  ", " src", "src ", "./ src", "src/ ", "\tsrc", "src\n"} {
		if args, err := BuildScanArgs(&ScanIntent{Paths: []string{path}}); err == nil {
			t.Errorf("path %q accepted: %q", path, args)
		}
	}
	for _, path := range []string{"src/my file.go", "src/ inner/file.go", "."} {
		if _, err := BuildScanArgs(&ScanIntent{Paths: []string{path}}); err != nil {
