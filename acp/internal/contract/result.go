...
// Summary mirrors jsonSummary usage, timing, and budget metadata. Human-readable
// result text lives in ReviewResult.Message. Optional token pointers distinguish
// older CLI output without token breakdowns from reported zero usage.
type Summary struct {
...
// ToolCalls contains aggregate tool usage reported by the OCR CLI.
...
type Coverage struct {
	Selected  []CoverageItem `json:"selected,omitempty"`
	Completed []CoverageItem `json:"completed,omitempty"`
	Reused    []CoverageItem `json:"reused,omitempty"`
	Failed    []CoverageItem `json:"failed,omitempty"`
	Waived    []CoverageItem `json:"waived,omitempty"`
}
