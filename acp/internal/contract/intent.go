...
func validateExtra(extra []string, allowed map[string]extraFlagSpec) error {
	for i := 0; i < len(extra); i++ {
		...
		if len(spec.enum) > 0 && !slices.Contains(spec.enum, strings.ToLower(value)) {
...
func normalizePath(p string) (string, error) {
	...
	// Unix absolute paths and Windows drive-root paths (\Windows) become a
	// leading slash after unification. filepath.IsAbs does not treat them as
	// absolute on the opposite OS, so reject the prefix explicitly.
	...
	// The CLI trims --path elements and drops empty ones. Reject padding so
	// the requested target cannot change or silently widen to the scan root.
	if cleaned != strings.TrimSpace(cleaned) {
		return "", fmt.Errorf("path has leading or trailing whitespace: %s", p)
	}
