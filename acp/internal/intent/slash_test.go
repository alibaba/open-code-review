...
func TestSlashRangeScreenRespectsFlagValues(t *testing.T) {
	for _, tc := range []struct {
		input string
		want  []string
	}{
		{"/scan --path ./a/../b", []string{"scan", "--path", "b"}},
		{"/scan --path=./a/../b", []string{"scan", "--path", "b"}},
		{"/scan --path a..b", []string{"scan", "--path", "a..b"}},
		{"/scan --path=a..b", []string{"scan", "--path", "a..b"}},
		{"/review --background a..b", []string{"review", "--background", "a..b"}},
		{"/review --background-file a..b", []string{"review", "--background-file", "a..b"}},
		{"/scan --background a..b", []string{"scan", "--background", "a..b"}},
	} {
		t.Run(tc.input, func(t *testing.T) {
			r, err := newTestParser(nil, nil).Parse(context.Background(), tc.input, NewState())
			if err != nil {
				t.Fatal(err)
			}
			got := requireIntent(t, r)
			want := append(tc.want, "--format", "json", "--audience", "human", "--color", "never")
...
func TestSlashCommitMissingValueHint(t *testing.T) {
	for _, input := range []string{"/review --commit", "/review --commit="} {
		r, err := newTestParser(nil, nil).Parse(context.Background(), input, NewState())
		if err != nil {
			t.Fatal(err)
		}
		if got := requireReject(t, r).Hint; got != "Example: /review --commit HEAD" {
			t.Fatalf("hint = %q", got)
		}
	}
}
