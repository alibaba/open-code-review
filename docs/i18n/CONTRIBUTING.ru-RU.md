...
│   ├── delegate/         # Генерация спецификаций делегирования
│   ├── diff/             # Разбор git-диффов
│   ├── gitcmd/           # Запуск git-подпроцессов
│   ├── llm/              # Клиент LLM API (Anthropic и OpenAI)
│   ├── llmloop/          # Общий цикл tool-use LLM
│   ├── mcp/              # MCP-клиент и регистрация инструментов
│   ├── model/            # Модели данных
│   ├── pathutil/         # Канонизация путей и проверка вложенности
│   ├── release/          # Проверки имён релизных артефактов
│   ├── scan/             # Полнофайловое сканирование (ocr scan)
│   ├── session/          # Управление сессиями ревью
│   ├── stdout/           # Подавление/перенаправление stdout
│   ├── suggestdiff/      # Построчные диффы предложений для CLI
