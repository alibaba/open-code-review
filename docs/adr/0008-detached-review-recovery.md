...
Provider request failures use bounded retry at the shared LLM request boundary: three total attempts with short exponential backoff, retrying only transient transport errors, timeouts, truncated responses, HTTP `408`, `429`, and `5xx` responses. Authentication failures, other `4xx` responses, and caller context cancellation stop immediately. This retry stays inside one review and never starts a second review invocation.
