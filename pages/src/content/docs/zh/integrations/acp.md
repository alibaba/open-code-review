...
- 使用支持 ACP v1 stdio 和自定义 Agent 启动命令的客户端。
  macOS 有本地自动化测试；Linux CI 运行测试、race 覆盖率和官方 Python SDK 冒烟测试。
  Windows 已实现 Job Object 和可取消 stdio，但当前 CI 仅交叉编译 Windows 产物。
  Windows 原生运行及图形客户端行为需要单独验证。
...
提供实验性源码构建支持。在 Linux riscv64 上，从仓库根目录分别运行 `make build`
和 `make -C acp build`，然后在客户端配置 `dist/opencodereview` 和 `dist/ocr-acp`
的绝对路径。当前发布流程不提供 riscv64 二进制，CI 也没有 riscv64 任务。

2026-09-17 使用 Go 1.26.5，在 Docker/QEMU 中对提交 `c9279e7` 验证了
OCR CLI 与 ACP 的构建、二进制启动及 ACP 核心无 race 测试。这是历史版本的兼容性
证据，不代表最新版本已通过同样验证。race 二进制可以编译，但 QEMU 运行时报告
`ThreadSanitizer: unsupported VMA range`；会启动 race 子进程的命令测试被排除。
该架构上的真实 RISC-V 硬件、Python SDK 冒烟、图形客户端及完整 OCR CLI 测试仍未验证。
...
Windows 下安装 Go 和 Make 后构建，产物为 `dist/ocr-acp.exe`。
在 PowerShell 中用 `Get-Command ocr` 查找 OCR，并在客户端配置中填写 `.exe` 路径；
JSON 路径使用正斜杠或转义反斜杠。取消时，有共享控制台则尝试发送 CTRL_BREAK，
随后通过 Job Object 清理残留进程。没有控制台时依靠强制终止，OCR 可能来不及返回部分 JSON 结果。
...
进度使用 OCR CLI 原有日志和最终 JSON，无需修改 CLI。展示最新上报的活动和总运行时间，不推测文件组阶段、阶段耗时或等待模型响应的时间。

单个 **OCR review** 或 **OCR scan** 条目展示进度和本地测量的运行时间。展开后可查看
**Command:**、工作目录及有长度上限的纯文本日志。诊断信息保留在详情中，不会替换
进度标题；命令与日志不会重复出现在会话正文。初始展开状态由客户端决定。

最终报告先展示完成状态、文件与问题数量、严重级别分布。每个问题使用带序号、
严重级别和文件位置的标题，下方展示分类、说明和带语言标记的代码块。建议代码明确
标注为尚未应用。问题只在最终正文展示一次。文件存在且位置有效时，可通过链接跳转；
文件不存在、行号越界或路径超出工作根目录时保留文本，不提供跳转。
具体渲染和点击行为取决于客户端版本。

部分失败详情和预算上限提示会保留。报告末尾显示可用的累计 token 和 OCR 报告的耗时。
执行详情还会展示 OCR 提供的工具调用总数及输入、输出、缓存 token；缺少的数据不补造。
实时计时测量的是本地执行时间，可能与 OCR 报告的耗时不同；token 总数累计多次模型
请求，不是新生成回答的 token 数。
...
| 审查失败或只返回部分结果 | 展开 OCR review/scan 查看诊断，并阅读最终失败信息。检查 OCR 模型配置及提供方可用性。 |
