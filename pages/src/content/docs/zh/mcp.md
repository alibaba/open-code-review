...
进入 **Tools** 可以离线查看已配置工具。**Test connection** 在启动或连接前确认，只读取工具目录，新发现的工具默认未选择。每个工具显示实际生效权限，包含上级 deny、禁用状态和本次发现的定义变化。停用工具可离线完成；接受新定义需要重新连接，并将该工具重置为 `ask`。review 启动时仍会重新核验指纹。

选择 **Import (JSON / TOML)**，从文件导入或私密粘贴配置。粘贴内容不回显，**Ctrl-S** 进入预览，**Esc** 取消。支持 Cursor JSON 的 `mcpServers` 和 Codex TOML 的 `mcp_servers`。
...
连接预览页也提供 **Save disabled (no connection)**（不连接，保存为禁用）。
...
`0600` 权限适用于 Unix 类系统。Windows 使用从目录继承的访问控制列表（ACL），
而不是 Unix 权限位；请确保配置目录仅允许你的账户访问。
...
review 中存在可见 MCP 工具时，即使设置 `OCR_RAW_LOGGING=1`，OCR 也会关闭原始流量记录。敏感工具调用在会话中保留脱敏参数，省略可能重复包含参数的原生数据及对应文本和思考内容；模型继续运行所需的原始上下文保持完整。MCP 子进程复用 SDK 的退出时限，review 结束时并发关闭客户端并限制等待时间。
