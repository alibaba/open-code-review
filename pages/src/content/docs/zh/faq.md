...
OCR 隔离 per-file 失败，使一个有问题的文件不会拖垮 20 文件的评审，部分结果也总会在退出前发布。但聚合退出码会区分运行不完整的原因：仅当每个选中条目都完成（或未完成的条目只是被 `--max-tokens-budget` 截断，报告为 `failed(budget)`）时才为 `0`；只要任何条目因真实原因（超时、provider 错误）失败，即为非零退出，即使其余全部成功。这样 CI 仅凭退出状态就能发现不完整的评审。查看 JSON 模式的 `warnings` 数组或文本模式的 stderr，看哪些文件失败了。
