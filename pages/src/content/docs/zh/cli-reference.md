...
一组文件的标准评审完成后，`ocr review` 总会为该组追加运行一次**对抗性审查（adversarial pass）**——一场全新的对话，使用同样的上下文工具，质疑所选设计、压测本次变更的假设（并发与竞态、鉴权与注入、数据丢失与回滚、错误处理、资源泄漏），并报告标准轮次遗漏的问题。它会把标准评审的发现作为“勿重复”上下文携带，其产出的评论与其余发现一样经过评审过滤器。

它没有开关：该 pass 是默认流水线的一部分。注意事项：

- 成本：每个文件组大约多一次 LLM 对话。
- 该 pass 尽力而为——如果它失败或提前停止（包括触发 `--max-tokens-budget`），运行会记录一条警告并保留标准评审的发现；该组的退出状态不变。
- 对抗 pass 无发现是合法结果：模型被告知在没有新问题时直接调用 `task_done`。
- `--preview` 不会运行它（preview 不执行任何 LLM 调用）。
