...
> **DNS 리바인딩 방어이지 접근 제어가 아닙니다.** 뷰어는 `Host` 헤더를 허용
> 목록(`localhost`, `127.0.0.1`, `::1`)과 대조합니다. 이는 DNS 리바인딩을 막지만,
> 리스너에 도달할 수 있는 피어는 `Host: localhost`를 보낼 수 있습니다. 허용 목록은 인증이
> 아니며 연결할 수 있는 네트워크 피어를 제한하지 않습니다. 콕 집은 바인드 호스트(예:
> `--bind 192.168.1.10 -p 5483`)는 허용 목록에 더해집니다. **와일드카드**
> 바인드(`--bind 0.0.0.0` 또는 `--bind ::`)는 루프백이 아닌 호스트를 추가하지 않습니다.
> 뷰어를 노출할 때 특정 비루프백 `Host` 헤더를 허용하려면
