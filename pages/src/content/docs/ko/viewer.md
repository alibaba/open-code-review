...
번에 해당합니다. 카드는 작업 종류별로 색이 달라, 어느 단계가 실행을 지배했는지
한눈에 보입니다.
...
코드도 예외가 아닙니다. 이 기록은 전부 여러분의 컴퓨터 안
`~/.opencodereview/`에만 있습니다. OCR은 어디에도 올리지 않습니다.
...
- 프로그램으로 후처리할 때(CI, 대시보드)는 `ocr review --format json
  --audience agent`를 쓰세요. 뷰어는 기계가 아니라 사람이 보라고 그립니다.
- 여러 세션을 가로질러 grep 하고 싶다면 JSONL 파일에 `jq`를 직접 쓰세요. UI에는
  아직 검색창이 없습니다.
