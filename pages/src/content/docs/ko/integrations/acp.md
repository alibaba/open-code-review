...
- ACP v1 stdio와 사용자 지정 Agent 명령을 지원하는 클라이언트를 사용하세요.
  macOS에서는 로컬 자동 테스트를, Linux CI에서는 테스트, race 검사와 Python SDK smoke 테스트를 실행합니다.
  Windows에는 Job Object와 취소 가능한 stdio가 구현되어 있지만 현재 CI는 교차 컴파일만 수행합니다.
  Windows 네이티브 실행과 그래픽 클라이언트 동작은 별도로 검증해야 합니다.
...
소스 빌드를 실험적으로 지원합니다. Linux riscv64에서 저장소 루트의 `make build`와
`make -C acp build`를 실행한 뒤 클라이언트에 `dist/opencodereview`와
`dist/ocr-acp`의 절대 경로를 설정하세요. 현재 릴리스는 riscv64 바이너리를
제공하지 않으며 CI에도 riscv64 작업이 없습니다.

2026-09-17에 Go 1.26.5와 Docker/QEMU를 사용하여 커밋 `c9279e7`의 OCR CLI 및
ACP 빌드, 바이너리 실행, race 없는 ACP 핵심 테스트를 통과했습니다. 이는 과거 버전의
호환성 검증이며 최신 버전의 검증을 의미하지 않습니다. race 바이너리는 컴파일되었지만
QEMU에서 `ThreadSanitizer: unsupported VMA range`로 실행되지 않았으며,
race 자식 프로세스를 실행하는 명령 테스트는 제외했습니다. 이 아키텍처의 실제 하드웨어,
Python SDK smoke, 그래픽 클라이언트 및 전체 OCR CLI 테스트는 아직 검증하지 않았습니다.
...
Windows에서는 Go와 Make를 설치한 뒤 빌드하고 `dist/ocr-acp.exe`를 사용하세요.
PowerShell의 `Get-Command ocr`로 OCR을 찾을 수 있습니다. JSON 경로에는 슬래시를 쓰거나
역슬래시를 이스케이프하세요. 취소 시 공유 콘솔이 있으면 CTRL_BREAK를 시도한 다음
Job Object로 남은 프로세스를 종료합니다. 콘솔이 없으면 강제로 종료하므로 일부 JSON 결과가 반환되지 않을 수 있습니다.
...
진행 상황은 기존 OCR CLI 로그와 최종 JSON을 사용하므로 CLI 수정이 필요하지 않습니다. 최근에 보고된 작업과 전체 경과 시간을 표시하며, 그룹 단계나 단계별 소요 시간, 모델 응답 대기 시간은 추정하여 표시하지 않습니다.

하나의 **OCR review** 또는 **OCR scan** 항목에 진행 상황과 로컬에서 측정한 실행 시간이
표시됩니다. 펼치면 **Command:**, 작업 디렉터리, 크기가 제한된 일반 텍스트 로그를 볼 수
있습니다. 진단 정보는 상세 내용에 유지되며 진행 제목을 덮어쓰지 않습니다. 명령과 로그는
대화 본문에 반복되지 않습니다. 처음 펼쳐지는지는 클라이언트가 결정합니다.

최종 보고서는 완료 상태, 파일 및 발견 사항 수, 심각도별 개수로 시작합니다. 각 발견 사항의
제목에는 번호, 심각도, 파일 위치가 있고, 그 아래에 분류, 설명, 언어가 지정된 코드 블록이
표시됩니다. 제안 코드는 아직 적용되지 않았음을 명시합니다. 발견 사항은 최종 본문에 한 번만
나타납니다. 유효한 파일 위치는 링크로 제공하며, 없는 파일, 범위를 벗어난 줄, 작업 루트 밖의
경로는 일반 텍스트로 유지합니다. 렌더링과 클릭 동작은 클라이언트 버전에 따라 다릅니다.

부분 실패 상세 정보와 예산 한도 알림은 유지됩니다. 보고서 끝에는 제공된 누적 토큰 수와
OCR이 보고한 소요 시간이 표시됩니다. 실행 상세에는 OCR이 제공하는 도구 호출 합계와
입력·출력·캐시 토큰 수도 표시하며, 없는 내역을 만들어 넣지 않습니다. 실시간 타이머는 로컬
실행 시간을 측정하므로 OCR 보고 시간과 다를 수 있습니다. 토큰 합계는 모든 모델 요청을
포함하며 새로 생성된 답변의 토큰만 세는 값이 아닙니다.

클라이언트의 취소 기능으로 중지하세요. 어댑터는 작업을 취소하고
...
| 리뷰가 실패하거나 일부 결과만 반환됨 | OCR review/scan을 펼쳐 진단 정보를 확인하고 최종 결과의 실패 상세 정보를 읽으세요. OCR의 모델 설정과 제공자 가용성을 확인하세요. |
