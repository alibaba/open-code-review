...
AI 기반 개발은 편리함을 제공하므로 적극적으로 활용하시기 바랍니다. 하지만 AI가 생성한 코드를 검토 없이 그대로 제출하는 것은 허용되지 않습니다. AI 출력물의 중복이나 문제점을 수정하지 않으면 개발자와 검토자 간의 협업 효율성이 크게 저하될 뿐만 아니라, 풀 리퀘스트 처리에도 차질이 생깁니다.

따라서 개발 과정에서 AI를 사용할 때는 다음 규칙을 반드시 준수해야 합니다.

### 규칙:

1. **최초 이슈 또는 풀 리퀘스트 제출 시 AI/LLM 사용 여부와 사용한 도구/모델을 명시해야 합니다.**
2. AI가 작성한 모든 코드 라인을 이해하고 AI의 작업 내용을 정확히 파악해야 합니다.
3. 검토자가 변경 이유를 문의할 경우, 해당 변경 사항이 본인이 작성했든 AI가 작성했든 관계없이 이유를 설명해야 합니다. 모든 관리자 질문과 풀 리퀘스트 검토 의견에 직접 답변해야 하며, AI/LLM을 사용해서는 안 됩니다.
4. 풀 리퀘스트(PR)에 `AI 생성 -> 수정 -> 수정 -> 수정`과 같은 과정이 반복되지 않도록 하십시오. 이는 AI가 생성한 코드를 검토하지 않고 문제가 발생한 후에 AI가 수정하도록 두는 악순환을 초래합니다.
5. AI/LLM이 생성한 코드, 텍스트 등을 검토를 요청하기 전에 반드시 직접 검토하십시오.
6. "Assisted-by", "Co-developed-by" 등의 커밋 푸터를 포함하여 커밋 기록에 AI/LLM의 기여를 명시하지 마십시오.
7. 장황한 커밋 메시지를 작성하지 마십시오. 중요한 정보는 커밋 메시지가 아닌 풀 리퀘스트 요약에 포함해야 합니다.
8. 위의 사항들을 준수하기 어렵거나 원하지 않는 경우, 질문이나 풀 리퀘스트를 닫아주십시오.
