...
- **이슈·티켓 조회** — 연결된 Jira나 GitHub 이슈를 Agent가 직접 가져와,
  변경이 적힌 요구 사항에 맞는지 확인하게 합니다.
...
많은 도구를 노출한다면 `tools`에 허용 목록을 지정하세요. 도구가 적고 날카로울수록
Agent가 흐트러지지 않고 토큰 비용도 줄어듭니다. 목록에 적었지만 서버가 실제로
제공하지 않는 이름은 경고와 함께 건너뜁니다. 그래서 오타는 아무 일도 일어나지
않는 대신 stderr에 드러납니다.
