...
잘린 라운드는 숨겨지지 않고 보고됩니다. provider가 `finish_reason: length`로 끝낸 응답은 해당 실행의 `warnings`에 `response_truncated` 항목을 추가합니다(`json`, `sarif`, `text` 출력 모두에서 확인 가능). 여기에는 몇 라운드인지, 어떤 상한에 도달했는지, 도구 호출을 몇 번이나 했는지가 적힙니다. `comments: 0`인 리뷰에 이런 경고가 붙어 있다면 상한을 올려 다시 실행할 가치가 있습니다:
