...
대조는 **공백을 가리지 않으며**, 각 스니펫을 두 가지 읽기로 차례로 시도합니다. 먼저
파일에 적힌 그대로, 그다음에는 diff에서 그대로 옮겨 온 스니펫을 위해 줄마다 맨 앞의
`+`/`-` 하나를 떼어 낸 형태입니다. 원문 그대로의 형태를 먼저 시도하기 때문에 코드에
속한 맨 앞 `-`(예: YAML 목록 항목)를 diff 표시로 잘못 읽지 않습니다. 끝내 찾지 못하면
