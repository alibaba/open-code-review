...
1. **hunk의 새 쪽** — 연이어 붙은 **문맥 줄과 추가된 줄**의 묶음(삭제된 줄만 있거나
   변경 없는 줄만 있는 묶음은 제외)에서 찾아 새 파일 기준 라인 번호와 `side=RIGHT`를 얻습니다.
   실패하면 **hunk의 옛 쪽**(문맥 줄과 삭제된 줄)을 다시 시도해 옛 파일 기준 라인
   번호와 `side=LEFT`를 얻습니다.
...
해결된 `side`도 기계가 읽는 출력에 포함됩니다. `RIGHT`는 변경 후 파일, `LEFT`는 삭제된 코드가 있던 변경 전 파일을 뜻하며, 해결되지 않은 코멘트에서는 생략됩니다.
