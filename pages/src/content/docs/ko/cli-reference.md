...
그룹의 표준 리뷰가 완료되면 `ocr review`는 해당 그룹에 대해 **적대적 리뷰 패스(adversarial pass)**를 항상 한 번 더 실행합니다. 동일한 컨텍스트 도구를 사용하는 완전히 새로운 대화로, 선택된 설계에 이의를 제기하고 변경 사항의 가정(동시성과 경쟁 상태, 인가와 인젝션, 데이터 손실과 롤백, 오류 처리, 리소스 누수)을 스트레스 테스트하며 표준 라운드가 놓친 문제를 보고합니다. 표준 리뷰의 발견 사항은 "반복 금지" 컨텍스트로 전달되며, 이 패스가 남긴 코멘트는 다른 발견 사항과 동일한 리뷰 필터를 거칩니다.

별도의 플래그는 없습니다. 이 패스는 기본 파이프라인의 일부입니다. 참고 사항:

- 비용: 파일 그룹당 LLM 대화가 약 한 번 추가됩니다.
- 이 패스는 best-effort입니다. 실패하거나 조기 중단되면(`--max-tokens-budget` 소진 포함) 경고를 기록하고 표준 리뷰의 발견 사항을 유지하며, 그룹의 종료 상태는 바뀌지 않습니다.
- 적대적 패스가 아무것도 찾지 못하는 것도 정상적인 결과입니다. 새로운 문제가 없으면 `task_done`을 호출하도록 모델에 지시합니다.
- `--preview`에서는 실행되지 않습니다(프리뷰는 LLM 호출을 하지 않습니다).
