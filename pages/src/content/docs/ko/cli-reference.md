...
| `--max-tokens <n>` | — | 설정 또는 템플릿 기본값 | 서브태스크당 프롬프트(입력) 토큰 상한이며 템플릿 기본값은 `200000`입니다. 이 실행에 한해 저장된 `max_tokens` 설정을 덮어씁니다. 출력 상한은 바뀌지 않습니다. `--max-completion-tokens`를 참고하세요. |
| `--max-completion-tokens <n>` | — | 설정 또는 템플릿 기본값 | 요청별 provider 완성(**출력**) 토큰 상한이며 템플릿 기본값은 `16384`입니다. 이 실행에 한해 저장된 `max_completion_tokens` 설정을 덮어씁니다. `--max-tokens`와 별개이므로 프롬프트 상한을 올려도 출력 상한은 함께 늘지 않습니다. 연쇄 사고가 `16384`토큰을 넘는 추론 모델에서는 응답이 중간에 잘리는 것을 막기 위해 이 값을 올리세요. |
