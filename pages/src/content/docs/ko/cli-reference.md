...
| 코드 | 뜻 |
|---|---|
| `0` | 리뷰가 끝났습니다. 모든 선택 항목이 완료되었거나, 완료되지 못한 항목이 `--max-tokens-budget` 중단(`failed(budget)`로 보고)뿐인 경우입니다. |
| `1` | 치명적 오류입니다. 잘못된 플래그, LLM 엔드포인트 해석 실패, 그룹별 서브 Agent 전멸 등이며 오류 내용은 stderr에 출력됩니다. 실행이 *부분적*인 경우(타임아웃이나 provider 오류 등 실제 실패로 하나라도 실패하고 나머지는 완료된 경우)에도 사용됩니다. 부분 결과는 0이 아닌 종료 전에 항상 게시되므로 JSON 소비자는 커버리지 진단을 잃지 않습니다. |
