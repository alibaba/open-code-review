...
Select **Tools and individual permissions** to inspect configured tools without connecting. **Test connection / refresh tools** asks before starting or contacting the server and only reads the catalog. Newly discovered tools remain unselected. Each tool shows its effective permission, including parent deny, disabled state and observed definition drift. Revoking a tool works offline. Accepting a new definition reconnects and resets that tool to `ask`. A saved fingerprint is checked again at review startup.

Use **Import connection (JSON / TOML)** to choose a file or paste a configuration privately. Pasted content is hidden; **Ctrl-S** opens the preview and **Esc** cancels. Cursor JSON uses `mcpServers`; Codex TOML uses `mcp_servers`.
...
The connection preview also offers **Save disabled without connecting**.
