...
[Rule resolution per file](#rule-resolution-per-file) — one catch-all `**/*`
entry yields `java.md` for a `.java` file, `python.md` for a `.py` file, and
`default.md` for an unrecognized extension. `merge_system_rule` works in all
three user layers (`--rule`, `<repo>/.opencodereview/rule.json`, and
`~/.opencodereview/rule.json`).

It merges the **system** layer only. Multiple *user* entries matching the same
file still resolve first-match-wins (see discussion
[#633](https://github.com/alibaba/open-code-review/discussions/633)), and a
matching layer still shadows lower user layers — `merge_system_rule` never
stacks several user rules.
