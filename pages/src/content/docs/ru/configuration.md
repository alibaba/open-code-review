...
Два ключа верхнего уровня в `~/.opencodereview/config.json` позволяют настроить
приблизительную оценку токенов для `ocr review` и `ocr scan`:

| Ключ | По умолчанию | Значение |
|---|---|---|
| `estimation_overhead_tokens` | `2000` | Оценка накладных расходов промпта на вызов сверх токенов содержимого. |
| `estimation_output_tokens_per_round` | `700` | Оценка выходных токенов на раунд `MAIN_TASK`. |

Оба ключа принимают неотрицательные целые числа. Значение `0`, отсутствие ключа
или `ocr config unset` восстанавливают значение по умолчанию. Например:
...
Это примеры калибровки, а не рекомендуемые значения для конкретной модели.
Сравните оценку с фактическим расходом в типичных запусках и скорректируйте её;
OCR не определяет эти значения автоматически по имени модели. Оценка по-прежнему
предполагает семь раундов `MAIN_TASK` на файл, а PLAN, удаление дубликатов и
сводка сохраняют собственные оценки выходных токенов.

Настройки влияют на оценку перед запуском и проверку бюджета перед отправкой
следующего файла при использовании `--max-tokens-budget`. Они не меняют расход,
сообщаемый API, или лимиты токенов в запросах; оценки остаются приблизительными.
