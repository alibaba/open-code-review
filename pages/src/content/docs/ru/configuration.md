...
API-ключ. Если не задан ни `providers.<name>.api_key`, ни `api_keys`, ни
`api_key_cmd`, OCR использует соответствующую переменную окружения.
...
`api_keys` перечисляет дополнительные ключи того же провайдера. Когда запрос получает ответ о лимите использования (HTTP 429 или 402 при исчерпанном балансе), следующий повтор идёт со следующим ключом, и остальные запросы этого запуска тоже начинаются с него. `Retry-After` длиннее 8 секунд говорит о том, когда сбросится лимит исчерпанного ключа, поэтому перед переходом к следующему ключу его не выжидают. Другие ошибки, например 401 для отклонённого ключа, ключ не переключают.

Переключение работает в пределах обычного бюджета из 6 попыток на запрос. Для 429, который и так повторяется, запросов не добавляется. 402 обычно сразу завершает запрос, а с `api_keys` повторяется со следующим ключом в пределах этого бюджета. Поэтому один запрос проверяет не больше 6 ключей, а ключи после шестого используются только последующими запросами.
...
Первым используется `api_key` или, если вместо него задан `api_key_cmd`, вывод этой команды, а затем по порядку `api_keys`. Если не задано ни то, ни другое, первым становится первый элемент `api_keys`, а переменная окружения провайдера не используется. `api_keys` работает как для встроенных, так и для пользовательских провайдеров.
...
`api_key_cmd`; иначе используется первый элемент `api_keys`; и только если не
задано ничего из этого, OCR возвращается к переменной окружения провайдера.
