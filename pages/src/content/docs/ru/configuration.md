...
### Лимит одновременных запросов провайдера

`max_in_flight` ограничивает, сколько LLM-запросов один запуск `ocr review`
одновременно держит в работе к одному провайдеру. Учитываются запросы, чья
HTTP-попытка выполняется, и ответы, тело которых ещё читается. Планирование,
основной обзор, дополнительный раунд, сжатие памяти, перепривязка комментариев
и фильтр обзора делят один лимит. Повторные попытки SDK освобождают слот до
паузы перед повтором, а потоковые ответы держат слот до конца потока.

Это настройка провайдера (в отличие от `timeout_sec`, `llm.max_in_flight`
не существует):
...
По умолчанию не задано (`0`) — ограничения нет. На `ocr scan` и `ocr llm test`
не влияет.
