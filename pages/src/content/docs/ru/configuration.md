...
Часто используемые команды можно сократить с помощью `ocr alias`. Псевдонимы
хранятся в том же пользовательском файле конфигурации и разворачиваются до
обычной маршрутизации команд.
...
Текст команды псевдонима — один аргумент в кавычках; он не должен начинаться с
`ocr`. Дополнительные аргументы после псевдонима добавляются в конец сохранённой
команды. Управление псевдонимами:
...
Имя псевдонима должно начинаться с буквы или цифры и может содержать буквы,
цифры, `-` и `_`. Оно не может затенять встроенные команды, такие как `review`
или `config`.

Соответствующая запись в JSON:
