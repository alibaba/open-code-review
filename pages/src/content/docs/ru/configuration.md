...
### Привязка сессии для кэширования промптов

OCR выводит ключ привязки кэша промптов для каждого LLM-диалога,
ограниченный сессией ревью и задачей внутри неё
(`<id-сессии>-<тип-задачи>-<хеш-области>`). Кэши промптов сопоставляются
по префиксам, поэтому ключи на уровне диалога удерживают каждый растущий
диалог (например, цикл инструментов ревью одного файла) на одном узле
кэша, вместо того чтобы прижимать весь запуск к одному «горячему» ключу;
префикс с ID сессии позволяет сопоставлять логи кэша на стороне
провайдера с записями `ocr session`.

Чтобы включить эту возможность, вставьте шаблонную переменную
`{ocr_session_key}` в значения `extra_headers` или `extra_body` там, где
провайдер ожидает ключ — OCR подставит ключ диалога в каждый запрос, а
без настройки ничего не отправляет:

```bash
# OpenAI: поле prompt_cache_key в теле запроса
ocr config set providers.openai.extra_body '{"prompt_cache_key": "{ocr_session_key}"}'

# Шлюзы с маршрутизацией по заголовку (Cloudflare, Fireworks, Mistral обычно используют x-session-affinity)
