...
### Лимит промпта для одного файла

По умолчанию OCR ограничивает промпт для каждого файла 58 888 токенами.
Увеличьте этот лимит для модели с большим контекстным окном, сохранив `max_tokens`:
...
`--max-tokens` для разового переопределения без изменения сохранённой конфигурации:
...
Флаг запуска имеет приоритет над `max_tokens`; если ни один из них не задан, OCR
использует значение по умолчанию из встроенного шаблона задач. Этот лимит
устанавливается для каждого файла и не зависит ни от ограничения на выходные токены
модели, ни от `--max-tokens-budget`, который ограничивает общее число токенов за
запуск. Чтобы вернуть встроенное значение по умолчанию, выполните
