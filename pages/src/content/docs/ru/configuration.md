...
`max_completion_tokens` — это предел дополнения (**вывода**) провайдера, который
отправляется с каждым запросом; в обоих встроенных шаблонах он равен `16384`.
Это намеренно отдельный от `max_tokens` параметр: повышение предела промпта для
модели с большим контекстным окном не должно увеличивать объём, который
запрашивается у модели на генерацию.

```bash
ocr config set max_completion_tokens 65536
ocr config unset max_completion_tokens   # обратно к встроенному значению
```

`--max-completion-tokens` переопределяет сохранённое значение для одного запуска:
...
Приоритет: флаг командной строки > сохранённое значение > встроенный шаблон.
Повышайте предел для рассуждающих моделей, чья цепочка размышлений естественно
превышает `16384` токенов: ровно на этой длине ответ обрезается посередине
анализа, после чего модель часто отбрасывает уже найденные кандидаты и не
возвращает ничего.

Обрезанный раунд сообщается, а не скрывается. Любой ответ, который провайдер
завершил с `finish_reason: length`, добавляет запись `response_truncated` в
`warnings` этого запуска (видна в выводе `json`, `sarif` и `text`). В ней указаны
номер раунда, достигнутый предел и число выполненных вызовов инструментов. Если
ревью вернуло `comments: 0` вместе с таким предупреждением, стоит повторить его
с повышенным пределом:
