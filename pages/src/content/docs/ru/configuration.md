...
По умолчанию OCR до пяти раз повторяет запросы при временных ошибках,
распознанных SDK. Параметр `max_retries` ограничивает этот бюджет. Значение
задаёт число повторов после первого запроса: `0` отключает повторы, а `1`
разрешает не более двух HTTP-запросов.

Параметр можно задать глобально для прежнего LLM-эндпоинта или для отдельного
провайдера:
...
Значение должно быть неотрицательным целым числом. Если параметр не задан,
сохраняется значение по умолчанию — пять повторов.
