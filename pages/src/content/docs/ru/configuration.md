...
Параметр `retry_codes` позволяет OCR повторять такие запросы через общую
ограниченную политику повторных попыток для всех LLM-протоколов.
...
Допускаются только дополнительные HTTP-коды 4xx. Общая политика OCR уже
...
предупреждение и не сохраняет их в конфигурации. Ответы 5xx также обрабатываются
общей политикой повторных попыток, поэтому такие коды нельзя добавлять в
`retry_codes`. Один запрос к provider выполняется не более трёх раз.
