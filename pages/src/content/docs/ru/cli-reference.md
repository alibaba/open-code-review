...
При публикации OCR строит отдельные наборы строк для старой и новой сторон
diff. Inline-комментариями становятся только находки, весь диапазон которых
присутствует исключительно на новой стороне. Находки на старой стороне,
неполные и неоднозначные диапазоны (включая одинаковый номер строки на обеих
сторонах) сохраняются в сводке pull request.
