...
`ocr` поставляется с man-страницей. После установки её можно открыть так:

```sh
man ocr
```

Установщик `install.sh` устанавливает страницу автоматически. Другие способы
установки (npm, сборка из исходников) этого не делают — сгенерируйте страницы
сами и положите их в свой `MANPATH`:
