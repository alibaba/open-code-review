...
(`ocr --no-color review` и `ocr review --no-color` равнозначны).
...
| `--no-color` | выключено | Сокращение для `--color=never`. |
...
`NO_COLOR` (любое непустое значение, согласно [no-color.org](https://no-color.org))
и `TERM=dumb` также отключают цвет. Явный `--color` или `--no-color` имеет
приоритет над ними: флаг — это решение для одного запуска, а переменная
окружения — постоянное предпочтение.
