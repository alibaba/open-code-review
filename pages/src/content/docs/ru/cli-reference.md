...
| Код | Значение |
|---|---|
| `0` | Ревью завершено. Либо все выбранные элементы завершились, либо незавершённые были остановлены только лимитом `--max-tokens-budget` (помечены как `failed(budget)`). |
| `1` | Критическая ошибка — неверные флаги, не удалось определить эндпоинт LLM, ошибкой завершились все субагенты по файлам и т. п. Текст ошибки выводится в stderr. Также возвращается, когда ревью *частичное*: хоть один элемент упал по настоящей причине (таймаут, ошибка provider), даже если остальные завершены. Неполные результаты публикуются до ненулевого выхода, поэтому JSON-потребители не теряют диагностику покрытия. |
