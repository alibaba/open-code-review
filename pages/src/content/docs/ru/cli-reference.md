...
| `--diff-only` | — | `false` | Проверяет только переданный diff и делает ровно один основной запрос к модели для каждой локально определённой группы. Отключает инструменты контекста репозитория и MCP, планирование, группировку через LLM, фильтрацию комментариев, перемещение через LLM, сжатие памяти и дополнительные раунды; оставляет только `code_comment` и `task_done` для структурированного вывода. |
...
> `--diff-only` переопределяет поведение `--effort` и `--max-tools`. За счёт отказа
> от контекста репозитория и глубины ревью он гарантирует один запрос к модели на
> локальную группу; длительность этого запроса по-прежнему зависит от выбранной
> модели и провайдера.
