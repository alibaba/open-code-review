...
| `--background-file <path>` | `-B` | — | Путь к Markdown-файлу, используемому как контекст ревью. Можно комбинировать с `--background`, чтобы использовать и inline-текст, и файл одновременно. |
| `--exclude <patterns>` | — | — | Разделённые запятыми паттерны в стиле gitignore для исключения из ревью; объединяются с исключениями из `rule.json` (описано также в разделе `ocr scan`). |
| `--max-tokens-budget <n>` | — | `0` (без лимита) | Ограничивает общее использование токенов (вход + выход) за весь запуск ревью. После превышения диспетчеризация останавливается, а оставшиеся файлы помечаются как `failed(budget)`. Частичные результаты всё равно публикуются, процесс завершается с кодом `0`; ненулевой код — только если не удалось выполнить все выбранные элементы. |
