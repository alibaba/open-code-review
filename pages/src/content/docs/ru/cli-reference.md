...
| `--post-to-pr` | — | `false` | Публикует результаты ревью в соответствующем pull request GitHub. Требует режим диапазона (`--from` и `--to`) и token GitHub. |
| `--github-token <token>` | — | — | Token GitHub для публикации комментариев pull request. Если не указан, используется переменная окружения `GITHUB_TOKEN`. |
...
##### Публикация результатов в pull request GitHub

Используйте `--post-to-pr`, чтобы явно включить публикацию. Нужен режим
диапазона с `--from` и `--to`; token передаётся через `--github-token` либо
читается CLI из `GITHUB_TOKEN`.

CLI находит единственный открытый pull request, у которого совпадают базовая
ветка и проверенный head commit. Номер pull request указывать не требуется.
Во время определения расположения OCR записывает, со старой или новой стороны
изменённого файла получена каждая находка. При публикации OCR также строит
набор строк diff pull request. Находка становится inline-комментарием, только
если она получена с новой стороны и весь её диапазон входит в проверенный,
полный hunk правой стороны diff. Находки со старой стороны, находки без данных
о стороне, недопустимые диапазоны и диапазоны, которые нельзя проверить по
полному patch, сохраняются в сводке pull request. Если сторона известна, одно
лишь числовое пересечение диапазонов строк не делает находку неоднозначной.

Inline-комментарии и сводки отправляются детерминированными пакетами, которые
можно безопасно возобновить. Перед каждой записью OCR проверяет, что pull
request всё ещё открыт, его базовая ветка и head совпадают, а текущая вершина
базы вместе с head даёт ровно тот merge-base commit, который записан в
завершённом ревью. Вершина базы может измениться только при неизменном
merge-base. Существующие последовательности inline и fallback возобновляются
без дублирования находок и без повторной inline-попытки для пакета, который уже
перешёл к сводке.

Локальный результат ревью создаётся до доставки в GitHub. Если любой шаг
публикации завершается ошибкой, OCR сохраняет этот вывод, но выходит с ненулевым
статусом, чтобы скрипты и CI не приняли сбой доставки за успех.

Token должен иметь доступ к целевому репозиторию и права на чтение и запись pull
request. Для fine-grained personal access token или GitHub App выдайте право
**Pull requests: Read and write**; для classic personal access token нужен
подходящий scope `repo` или `public_repo`. В GitHub Actions задайте
`pull-requests: write`. Это право также покрывает issue-комментарии, используемые
для находок без допустимого inline-расположения.

Для GitHub.com оставьте `GITHUB_SERVER_URL` и `GITHUB_API_URL` незаданными
(либо используйте `https://github.com` и `https://api.github.com`). Для GitHub
Enterprise Server задайте HTTPS URL в `GITHUB_SERVER_URL`; API по умолчанию —
`https://HOST/api/v3`. Явно заданный `GITHUB_API_URL` также должен использовать
HTTPS и тот же host, что и server URL.
