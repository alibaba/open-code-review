...
### Адверсариальный этап ревью

После завершения стандартного ревью группы `ocr review` всегда выполняет для неё один дополнительный **адверсариальный этап (adversarial pass)** — отдельный диалог с теми же контекстными инструментами, который ставит под сомнение выбранное решение, стресс-тестирует предположения изменения (параллелизм и гонки, авторизацию и инъекции, потерю данных и откат, обработку ошибок, утечки ресурсов) и сообщает о том, что стандартные раунды пропустили. Находки стандартного ревью передаются ему как контекст «не повторять», а его комментарии проходят тот же фильтр ревью, что и остальные находки.

Отдельного флага нет: этот этап — часть конвейера по умолчанию. Примечания:

- Стоимость: примерно один дополнительный диалог LLM на группу файлов.
- Этап выполняется по возможности: при сбое или досрочной остановке (включая срабатывание `--max-tokens-budget`) записывается предупреждение, находки стандартного ревью сохраняются; статус завершения группы не меняется.
- Пустой адверсариальный этап — допустимый результат: модель проинструктирована вызывать `task_done`, если ни одно из её сомнений не подтвердилось.
- `--preview` его не запускает (превью не выполняет вызовов LLM).
