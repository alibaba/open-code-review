...
Для провайдера на локальном CLI (`claude-code` / `codex`, протокол `claude-cli` /
`codex-cli`) строка `URL:` заменяется строкой `Backend:` с именем бинарника CLI,
потому что у CLI-бэкенда нет URL. Для `bedrock` она заменяется строками
`Region:` / `Profile:`.
