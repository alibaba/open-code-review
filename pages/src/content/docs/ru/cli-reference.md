...
| `--delta-from <session-id>` | — | — | Ревью новой версии изменения с повторным использованием результатов этой сессии для файлов, чей diff не изменился. См. [Ревью новой версии изменения](#ревью-новой-версии-изменения). |
...
### Ревью новой версии изменения

Когда у изменения появляется новая версия — учтены замечания, сделан rebase, добавлен коммит, —
`--delta-from` проверяет только файлы с изменившимся diff, а для остальных повторно использует
...
Файл переиспользуется, только если его diff побайтово совпадает с diff, проверенным в предыдущей
сессии, поэтому его замечания и номера строк остаются верными. Оба раза используйте одну и ту же
базу: сдвиг базы меняет diff каждого файла. В остальном действуют правила `--resume`, кроме того, что
входные данные могут отличаться: только ревью диапазона или коммита, тот же репозиторий, правила и
фильтр файлов, без неявной смены provider или model. `--delta-from` нельзя сочетать с `--resume` или
`--preview`.

Запуск записывает предыдущую сессию как родительскую, перечисляет переиспользованные файлы в
`coverage.reused` и сообщает `"delta": true` в блоке `resume` JSON-вывода. Переиспользованные файлы
не проверяются заново, поэтому замечание в неизменённом файле, зависящее от кода в изменённом файле,
не перепроверяется; если это важно, запустите полное ревью.
