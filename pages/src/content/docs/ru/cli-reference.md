...
| `--max-tools <n>` | — | значение шаблона | Максимальное число раундов вызова инструментов для каждого файла. `0` использует значение шаблона (`100`); значения 1–49 повышаются до `50`; итоговое положительное значение точно задаёт предел. |
