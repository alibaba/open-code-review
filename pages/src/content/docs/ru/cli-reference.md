...
| `--max-tokens <n>` | — | значение конфигурации или шаблона | Предел токенов **промпта** для каждой подзадачи (по умолчанию `200000` для review). Переопределяет сохранённое значение `max_tokens` для этого запуска. На предел вывода не влияет — он задаётся отдельно параметром `--max-completion-tokens`. |
| `--max-completion-tokens <n>` | — | значение конфигурации или шаблона | Предел токенов дополнения (**вывода**) провайдера для каждого запроса; по умолчанию в шаблоне `16384`. Переопределяет сохранённое значение `max_completion_tokens` для этого запуска. Это отдельный от `--max-tokens` параметр, поэтому повышение предела промпта не увеличивает предел вывода. Повышайте его для рассуждающих моделей, чья цепочка размышлений превышает `16384` токенов — иначе ответ будет обрезан посередине. |
