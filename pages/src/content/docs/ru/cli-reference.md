...
| `--max-tokens <n>` | — | значение из конфигурации или шаблона | Предел числа токенов промпта для одного файла. Переопределяет сохранённую настройку `max_tokens` для этого запуска. |
