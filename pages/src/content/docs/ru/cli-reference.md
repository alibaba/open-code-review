...
| `--budget-preflight <warn\|confirm\|abort>` | — | `warn` | Определяет, что делать, когда расчётная стоимость токенов превышает бюджет. `warn` сохраняет прежнее неблокирующее поведение, `confirm` спрашивает перед запуском и завершается ошибкой (fail-closed), если нет интерактивного терминала, а `abort` завершает работу до создания сессии или вызова LLM. Требуется положительный бюджет токенов. |
...
`ocr scan` также принимает `--max-tokens-budget` и
`--budget-preflight <warn|confirm|abort>`. Предварительная проверка оценивает работу,
оставшуюся после исключений, фильтра по размеру и переиспользуемых контрольных точек
возобновления. Если оценка превышает бюджет, `warn` продолжает работу, `confirm` спрашивает
перед запуском, а `abort` завершает работу до создания сессии или вызова LLM. Оценка — это
приблизительное число токенов, а не специфичные для провайдера платёжные кредиты.
