...
видите только diff, _внесённый_ веткой функций, без несвязанных изменений,
...
- режим ревью должен совпадать: сессию диапазона нельзя возобновить как ревью
  коммита;
- разрешённый вход должен совпадать. _Написание_ ref не сравнивается (`abc1234`
...
`--budget-preflight <warn|confirm|abort>`. `warn` сохраняет прежнее неблокирующее
поведение и не запускает дополнительную admission preflight. Для `confirm` и `abort`
предварительная оценка учитывает только оставшуюся работу, для которой после исключений,
фильтра по размеру и переиспользуемых контрольных точек возобновления всё ещё нужен новый
LLM dispatch. Если эта оценка превышает бюджет, `confirm` спрашивает перед запуском, а
`abort` завершает работу до создания сессии или вызова LLM. Оценка — это приблизительное
число токенов, а не специфичные для провайдера платёжные кредиты.
