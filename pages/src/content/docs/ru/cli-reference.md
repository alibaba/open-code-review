...
| `ocr gate` | — | Проверяет сохранённый результат ревью для CI без вызова модели. |
...
Проверяет один сохранённый результат `ocr review --format json` без обращения к модели, репозиторию или API хостинга. Запуск команды включает проверку; коды завершения `ocr review` и его manifest не меняются.
...
|---|---|---|
| `--input <path>` | обязателен | Файл результата; `-` читает stdin. |
| `--fail-on-severity <level>` | отключён | Блокирует замечания уровня `critical`, `high`, `medium` или `low` и выше. Регистр и крайние пробелы игнорируются. Отсутствующий или неизвестный уровень означает неопределённый результат проверки. |
| `--expected-base <sha>` | отключён | Проверяет фактический base: для диапазона это merge-base, а не вершина запрошенной ветки. |
| `--expected-head <sha>` | отключён | Проверяет рассмотренный head. Оба флага принимают полный идентификатор из 40 или 64 шестнадцатеричных символов в нижнем регистре. Результат рабочего дерева не подтверждает неизменяемую версию. |
| `--format <text\|json>`, `-f` | `text` | JSON использует `ocr.gate/v1` и содержит ID запуска, политику и упорядоченные проверки со стабильными кодами причин. |

Только `pass` возвращает **0**. `fail` (подтверждённое нарушение порога) и `inconclusive` (недостаточно данных) возвращают **1** после вывода решения. Ошибки аргументов, открытия входного файла и записи также возвращают 1; при этом возможна только диагностика в stderr. Некорректный JSON выдаёт неопределённое решение. Если одновременно найдены блокирующее замечание и неполное покрытие, общий статус — `fail`, но сохраняются обе причины.

Покрытие и отправка замечаний проверяются всегда. Все selected должны находиться в completed или reused согласно существующему контракту resume. Failed, **включая остановки по бюджету**, waived и сбои всего запуска не проходят. Нулевой выбор также неопределён: manifest v1 не различает отсутствие изменений и исключение всех файлов. Пропущенные или противоречивые данные, старые форматы и неизвестные версии manifest не проходят. Успех относится только к выбранным файлам и не доказывает, что выбор охватил всё необходимое команде.

Число ошибок инструментов должно совпадать с подробностями. Любая записанная ошибка `code_comment` делает отправку неопределённой, даже если последующий вызов успешен: текущий результат не доказывает повторную отправку тех же замечаний. Ошибки поисковых запросов или чтения сами по себе не блокируют. Порог применяется ко всем исходным замечаниям, включая не опубликованные построчно; без `--fail-on-severity` важность не проверяется.

Используйте доверенные результаты, политику и ожидаемые ревизии. Команда не удостоверяет JSON, не запрашивает текущий PR head, не проверяет публикацию и не доказывает отсутствие дефектов. Без флагов ожидаемых ревизий версии не сравниваются. Сохраняйте исходный код завершения ревью в CI: успех gate не должен скрыть ошибку выполнения или публикации. Используйте новый путь результата для каждого запуска, поскольку сбой ревью может оставить старый файл. Сохраните код завершения, сохраните или загрузите результат даже при ошибке, затем выполните gate; CI должен завершиться ошибкой, если выполнение или gate не прошли. Повторная проверка результата не расходует токены модели.
