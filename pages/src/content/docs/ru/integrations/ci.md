...
## Необязательный CI gate

Обе интеграции могут запускать [`ocr gate`](../cli-reference/#ocr-gate) после публикации
ревью. По умолчанию gate выключен. Укажите сборку OCR с командой `ocr gate` через
`ocr_version` в GitHub или `OCR_VERSION` в GitLab. При включённом gate интеграция
проверяет доступность команды до вызова модели.

| Решение | Значение | Код завершения |
|---|---|---|
| `pass` | Есть полные данные о ревью всех выбранных файлов, и все включённые проверки пройдены. | `0` |
| `fail` | Важность замечания соответствует заданному порогу или превышает его. | `1` |
| `inconclusive` | Недостаточно данных о покрытии, проверяемых ревизиях, публикации комментариев или других условиях проверки. | `1` |

Перед ревью фиксируются ID коммитов merge base и head; те же ID передаются
ревью и gate. Проверяются все замечания из исходного JSON, включая направленные
в сводку и пропущенные при публикации как дубликаты. Остановка по бюджету,
элементы `waived`, отсутствие выбранных файлов, записанные сбои `code_comment` и
неизвестная версия manifest не позволяют пройти gate.

Для успеха задания нужны успешное ревью, публикация и решение `pass` от gate.
Попытки публикации и проверки gate выполняются до определения итогового статуса.
Ошибка публикации комментариев к строкам кода приводит к сбою задания, в том числе
при публикации резервной сводки. Публикация итоговой сводки должна быть подтверждена.
...
| `gate` | `'false'` | Включает общий CI gate. Значения `true` / `false` нечувствительны к регистру. |
| `fail_on_severity` | `''` | Завершает проверку с ошибкой при замечаниях заданного уровня (`critical`, `high`, `medium`, `low`) и выше. Требует `gate: 'true'`. Пустое значение пропускает проверку важности, сохраняя требования к покрытию и публикации. Регистр и пробелы по краям игнорируются. |
...
Добавьте параметры в `with:` существующего шага action:
...
При включённом gate проверяется весь диапазон от merge base до head. Чтение и
обновление чекпоинтов пропускаются независимо от `checkpoint_range`. Повторная
проверка полного диапазона в PR, который долго остаётся открытым, может увеличить расход токенов.
Маршрутизация комментариев и инкрементальная публикация сохраняются.

`gate_exit_code` содержит код завершения команды gate. Значение пустое, если
проверка выключена или выполнение до неё не дошло. При `upload_artifacts: 'true'`
(по умолчанию)
сохраняются `ocr-result.json` и `ocr-stderr.log`; после запуска gate к ним добавляются
`ocr-gate.json` и `ocr-gate-stderr.log`. Каждый вызов action использует отдельный
временный каталог.
...
| Комментарии ревью попадают не на те строки | Head PR или diff могли измениться во время ревью. Замечания, которые не удалось опубликовать в строках кода, включаются в итоговый комментарий. При `gate: 'true'` оставшиеся ошибки публикации комментариев к строкам кода приводят к сбою задания. Проверьте head, использованный для ревью, и позиции в diff, затем повторите запуск. |

Для диагностики проверьте загруженные `ocr-result.json` и `ocr-stderr.log`. После
запуска gate также доступны `ocr-gate.json` и `ocr-gate-stderr.log`.
...
- Скрипт `post_review.py` разбирает результат ревью в формате JSON и публикует
  замечания как обсуждения в diff GitLab, используя эндпоинт `versions` MR
...
Скопируйте конвейер и скрипт публикации в корень репозитория:
...
Оставьте `post_review.py` в корне репозитория или обновите путь к нему в конвейере.
...
> Требование GitLab о минимуме в 8 символов относится к маскированным переменным.
> Значения политики, например `OCR_GATE`, задавайте обычными немаскированными
> переменными. В примере `llm.use_anthropic` равен `false`; для моделей Anthropic
> Claude измените эту строку конфигурации.
...
Задавайте эти значения обычными немаскированными переменными. `OCR_GATE` по
умолчанию равен `false`; регистр `true` / `false` не учитывается. Поддерживаются
пороги `critical`, `high`, `medium`, `low`; регистр и пробелы по краям игнорируются.
Пустое значение пропускает проверку важности. Недопустимые значения вызывают
ошибку до установки и вызова модели. При выключенном gate параметр
`OCR_FAIL_ON_SEVERITY` сохраняет прежнюю проверку в скрипте публикации.
...
Задайте нужную npm-версию через `OCR_VERSION`. Для gate требуется версия с командой `ocr gate`.
...
`only: [merge_requests]` запускается при каждом обновлении MR. При `OCR_GATE=false`
можно сократить расходы, пропуская ревью при наличии заметки OCR. При `OCR_GATE=true`
после каждого обновления MR требуется новое ревью всего текущего диапазона изменений.
Следующий пример пропускает ревью при наличии заметки только при выключенном gate.
При таком пропуске последующие изменения остаются непроверенными до следующего запроса ревью.
...
Конвейер сохраняет артефакты с путями относительно проекта через `when: always`:
`.ocr/ocr-result.json`, `.ocr/ocr-stderr.log`, `.ocr/ocr-gate.json` и
`.ocr/ocr-gate-stderr.log`. Файлы gate пусты, если проверка выключена или выполнение
до неё не дошло. Статистика публикации доступна в dotenv-отчёте `.ocr/ocr-stats.env`.
Проверьте эти файлы на отладочном этапе:
