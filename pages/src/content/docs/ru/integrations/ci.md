...
| `min_severity` | `''` | Минимальная важность в отчёте, включая указанный уровень: `critical`, `high`, `medium` или `low`. Пустое значение отключает этот фильтр. Находки с отсутствующей или неизвестной важностью либо категорией сохраняются. |
| `exclude_categories` | `''` | Исключаемые категории через запятую: `bug`, `security`, `performance`, `maintainability`, `test`, `style`, `documentation`, `other`. Пустое значение отключает исключение категорий. Находки с отсутствующей или неизвестной важностью либо категорией сохраняются. |
...
Для `min_severity` и `exclude_categories` нужна версия CLI с поддержкой `--min-severity` и `--exclude-categories`. Сначала CLI фильтрует находки для отчёта, затем `route_severity_below` и `route_categories` определяют, какие из оставшихся находок попадут в сводку. Исходные находки сохраняются в сессиях. Изменение любого фильтра сбрасывает контрольную точку между push-событиями, поэтому следующий запуск проверяет полный диапазон. Правила и примеры приведены в [справочнике CLI](../cli-reference/).
