...
MCP работает fail closed в CI и других неинтерактивных средах. Инструмент
`ask` скрывается от модели и не выполняется, потому что prompt approval
недоступен. Показан может быть только инструмент, явно включённый в `tools`, с
совпадающим `tool_definition_sha256`, постоянным итоговым `allow` и без
global/server `deny`. Даже тогда независимый authorizer повторяет проверку
непосредственно перед `tools/call`.

Подготовьте и проверьте config через `ocr mcp tools` и `ocr mcp permissions` в
доверенном интерактивном терминале; не пытайтесь подтверждать вызовы в runner.
Неинтерактивные команды, способные подключаться или писать, требуют `--yes`.
Команда `ocr mcp` без subcommand выводит только замаскированный status/help.

Передавайте MCP secrets через CI environment variables минимального scope и
ссылайтесь на них как `${ENV_NAME}`. Не commit resolved tokens. Закрепите local
server package или remote endpoint, выдайте минимальные server/tool permissions
и меняйте credentials независимо от LLM и PR-posting tokens. MCP применяется к
`ocr review`, но не к `ocr scan`.

Полная permission matrix, миграция fingerprints, ограничения remote transport
и диагностика описаны на странице [Серверы MCP](../mcp/).
