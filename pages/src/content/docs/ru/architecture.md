...
default_path    — совпадение со встроенным исключением пути
```
...
5. `default_path` — последняя проверка: она сопоставляет встроенные исключения
   путей, включая шаблоны тестовых файлов (`**/*_test.go`,
   `**/*.test.{js,jsx,ts,tsx}`, `**/__tests__/**`, `**/*_test.py`,
   `**/*_spec.rb`, `**/*.test.ets`, …) и префиксы шумных каталогов в tracked
   diff (`vendor/`, `node_modules/`, `target/` и т. д.). Пользовательские
   шаблоны `include` обходят эту проверку.

Tracked diff внутри шумных каталогов остаётся в разобранном наборе diff до этой
проверки, поэтому preview и delegate выводят явные пути и причины. В
workspace-режиме untracked файлы в тех же каталогах всё ещё пропускаются в
`internal/diff/git.go` до синтеза diff всего файла.
