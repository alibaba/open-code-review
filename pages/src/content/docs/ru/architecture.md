...
1. **Определение строк** (в обработчике) — `existing_code` сопоставляется с
   ...
   `start_line` / `end_line` и `side` (`RIGHT` для файла после изменения,
   `LEFT` для удалённого кода в файле до изменения). Если сопоставление не удаётся,
   оба значения становятся равны `0`, а поле `side` отсутствует. Диапазон строк `0` служит неявным признаком
