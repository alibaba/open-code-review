...
| `**/*.m` | по умолчанию `matlab.md` — см. [определение содержимого для файлов `.m`](#content-sniffing-for-m-files) ниже. |
| *(fallback)* | `default.md` |
...
Расширение `.m` используется двумя не связанными друг с другом языками — MATLAB и
Objective-C, — поэтому одного сопоставления по пути недостаточно, чтобы их
различить. Перед тем как откатиться к `matlab.md` при совпадении с `**/*.m`,
OCR заглядывает в **первую непустую строку** файла:

| Если первая строка похожа на | Используемый документ правила |
|---|---|
| `#import`, `#include`, `#pragma`, `#if` (охватывает и `#ifdef`/`#ifndef`), `#define`, `@import`, `@interface`, `@implementation`, `@class`, `@protocol`, `//` или `/*` | `objc.md` |
| что-либо иное (включая случай, когда содержимое недоступно для анализа, например файл удалён) | `matlab.md` |

Открывающий комментарий в стиле C (`//` или `/*`) сам по себе является надёжным
признаком Objective-C: комментарии в MATLAB начинаются с `%`, а файл `.m` не
может по правилам MATLAB начинаться с символа `/`, поэтому эти два случая
однозначно различимы. Это важно, поскольку шаблон файла Xcode начинается с
баннера `//`, а в большинстве реальных проектов сначала идёт заголовок с
лицензией — сам `#import` редко оказывается на первой строке.

Мы намеренно не расширяли признак до одиночного `#`: в Octave, который тоже
использует расширение `.m`, символ `#` обозначает комментарий, поэтому такое
расширение привело бы к тому, что настоящий файл Octave/MATLAB ошибочно
определялся бы как Objective-C.

Содержимое читается **на той ревизии (ref), которая проверяется**, а не из
вашей рабочей копии: `ocr review --from/--to` читает через
`git show <to>:<path>`, а `--commit` — через `git show <commit>:<path>`,
поэтому определение работает корректно, даже если эта ревизия не извлечена
(checked out). У режима workspace-ревью, `ocr scan` и `ocr rules check` нет
ref, и они читают рабочую копию — именно то, что они и проверяют. Если файл
вообще не удаётся прочитать, разрешение откатывается к `matlab.md`.

Сейчас `objc.md` поставляется как копия общего чек-листа `default.md` — это
заглушка в исходном коде OCR (`internal/config/rules/rule_docs/objc.md`),
предназначенная для того, чтобы кто-то впоследствии заполнил её настоящими
рекомендациями по Objective-C. Поскольку файл компилируется в бинарник через
`go:embed`, для его изменения требуется пересборка OCR из исходников, а не
просто редактирование файла на диске. Если вам нужны рекомендации по
Objective-C уже сейчас, без пересборки, используйте запись в
[`.opencodereview/rule.json`](#rule-file-format-layers-1-3) уровня проекта,
соответствующую вашим путям `.m` (например, `ios/**/*.m`) — правила проекта
проверяются раньше системного уровня, поэтому они получают приоритет
независимо от результата определения.

`ocr rules check` сообщает о срабатывании определения в отдельной строке
`Note:` — `Pattern` всегда остаётся тем самым обычным шаблоном (glob), который
совпал:
