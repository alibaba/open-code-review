...
- `include` — необязательно. Glob-шаблоны, которые *обходят* встроенные
  стандартные шаблоны исключения (исключения тестовых файлов и каталогов
  зависимостей — см. ниже). Это
...
6. **`default_path`** — Путь совпадает со встроенным стандартным шаблоном
   исключения — тестовым файлом (`**/*_test.go`, `**/*.test.{js,jsx,ts,tsx}`,
   …) или каталогом зависимостей / выводом сборки (`**/node_modules/**`,
   `**/vendor/**`, …)? Исключается.
...
совпадает с двумя семействами путей — JSON-файл является единственным источником
истины:

- тестовые файлы, напр. `**/*_test.go`, `**/*.test.{js,jsx,ts,tsx}`,
  `**/*_spec.rb`, `**/test_*.py`
- каталоги зависимостей и вывод сборки, напр. `**/node_modules/**`,
  `**/vendor/**`, `**/target/**`, `**/__pycache__/**`,
  `**/package-lock.json`, `**/*.min.{js,css}`

Фильтрация шумных каталогов (`vendor/`, `node_modules/`, `target/`, …)
происходит раньше и на уровне diff в
[`internal/diff/git.go`](https://github.com/alibaba/open-code-review/blob/main/internal/diff/git.go),
однако тот чёрный список совпадает только с корнем репозитория. Именно этап
default_path ловит эти каталоги, вложенные на любую глубину: как только
`node_modules/` внутри каталога пакета закоммичен, `.gitignore` больше не
удерживает его вне diff, и эти шаблоны остаются последней линией обороны.

Чтобы **отревьюить** файл, совпадающий с одним из этих стандартных шаблонов,
