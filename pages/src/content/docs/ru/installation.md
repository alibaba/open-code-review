...
| `OCR_GITHUB_MIRROR` | не задана | Скачивать бинарник релиза через зеркало GitHub (например `gh-proxy.com`). |

Скрипт поддерживает `darwin` и `linux` на `amd64` / `arm64`.

#### Использование зеркала GitHub

В некоторых регионах доступ к GitHub может быть медленным. Задайте `OCR_GITHUB_MIRROR` как домен зеркала, чтобы загружать бинарник через него:
...
Зеркало покрывает только загрузку бинарника. Разрешение версии (когда `OCR_VERSION` не задана) по-прежнему обращается к GitHub API напрямую, а контрольная сумма `sha256sum.txt` всегда загружается с GitHub, а не с зеркала. Чтобы полностью пропустить разрешение версии, закрепите версию:

```bash
export OCR_VERSION='v1.2.3'
```

> **Примечание по безопасности:** Зеркало — это сторонний сервис, поэтому бинарник загружается с него. Чтобы сохранить гарантию целостности, `sha256sum.txt` всегда загружается напрямую с GitHub — подменённый бинарник не пройдёт проверку контрольной суммы. Если GitHub недоступен, установщик откатывается к загрузке файла контрольной суммы с зеркала и печатает предупреждение; в этом случае сверяйтесь с оригинальным `sha256sum.txt` на [странице релизов](https://github.com/alibaba/open-code-review/releases).
