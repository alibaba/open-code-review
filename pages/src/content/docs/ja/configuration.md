...
OCR は SDK が一時的と判断したエラーをデフォルトで最大 5 回リトライします。
`max_retries` を設定すると、このリトライ上限を変更できます。この値は最初の
リクエスト後のリトライ回数であるため、`0` はリトライを無効にし、`1` は HTTP
リクエストを最大 2 回まで許可します。

従来の LLM エンドポイント全体、または個別のプロバイダーに設定できます。
...
値は 0 以上の整数でなければなりません。未設定の場合はデフォルトの 5 回が維持されます。
