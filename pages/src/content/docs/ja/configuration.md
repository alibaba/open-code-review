...
`max_in_flight` は、1 回の `ocr review` 実行の中で同じ provider に対して
同時に保持する LLM リクエスト数の上限です。HTTP リクエスト実行中のものと、
レスポンスボディを読み込み中のものがカウントされます。計画、メインレビュー、
補足ラウンド、メモリ圧縮、コメント再配置、レビューフィルターのすべてが
この上限を共有します。SDK のリトライはバックオフ待機の前にスロットを
解放し、ストリーミングレスポンスはストリーム終了まで保持します。

provider ごとの設定です（`timeout_sec` とは異なり `llm.max_in_flight` はありません）:

```bash
ocr config set custom_providers.my-gateway.max_in_flight 4
```

デフォルトは未設定（`0`）で、制限なしを意味します。`ocr scan` と
`ocr llm test` には影響しません。
