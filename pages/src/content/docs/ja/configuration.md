...
OCR のデフォルトは、各ファイルレビューに対して 58,888 トークンのプロンプト上限です。
より大きなコンテキストウィンドウを持つモデルでは、`max_tokens` を保存して増やせます：

```bash
ocr config set max_tokens 200000
```

この設定は `ocr review` と `ocr scan` の両方に適用されます。保存された設定を変更せずに
一時的に上書きするには `--max-tokens` を使用します：
...
実行ごとのフラグは `max_tokens` より優先されます。どちらも設定されていない場合、OCR は
埋め込みのタスクテンプレートのデフォルトを使用します。この上限はファイル単位であり、
モデルの出力トークン上限や、実行全体のトークン使用量を制限する `--max-tokens-budget`
とは独立しています。埋め込みのデフォルトに戻すには `ocr config unset max_tokens` を
実行してください。
