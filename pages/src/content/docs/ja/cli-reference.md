...
グループの標準レビューが完了すると、`ocr review` は必ずそのグループに対して**敵対的レビューパス（adversarial pass）**を 1 回追加実行します。同じコンテキストツールを使うまったく新しい会話で、採用された設計に異議を唱え、変更が依存する前提（並行性と競合、認可とインジェクション、データ損失とロールバック、エラー処理、リソースリーク）をストレステストし、標準ラウンドが見逃した問題を報告します。標準レビューの発見は「繰り返さない」コンテキストとして渡され、このパスが挙げたコメントは他の発見と同じレビューフィルターを通ります。

フラグはありません。このパスはデフォルトのパイプラインの一部です。注意点：

- コスト：ファイルグループごとに LLM 会話が約 1 回追加されます。
- このパスはベストエフォートです。失敗または早期停止（`--max-tokens-budget` の消費を含む）した場合は警告を記録し、標準レビューの発見を保持します。グループの終了ステータスは変わりません。
- 敵対的パスが何も見つけられないのは正当な結果です。新しい問題がないときは `task_done` を呼ぶようモデルに指示しています。
- `--preview` では実行されません（プレビューは LLM 呼び出しを行いません）。
