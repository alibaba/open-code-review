...
| `--max-tokens <n>` | — | 設定またはテンプレートのデフォルト | サブタスクごとの**プロンプト**トークン上限（review のデフォルトは `200000`）。この実行で保存済みの `max_tokens` 設定を上書きします。出力の上限には影響しません。そちらは `--max-completion-tokens` が個別に制御します。 |
| `--max-completion-tokens <n>` | — | 設定またはテンプレートのデフォルト | リクエストごとの provider 補完（**出力**）トークン上限。テンプレートのデフォルトは `16384`。この実行で保存済みの `max_completion_tokens` 設定を上書きします。`--max-tokens` とは別の制御なので、プロンプト上限を上げても出力上限は一緒に広がりません。連鎖思考が `16384` トークンを超える推論モデルでは、応答が途中で切られるのを防ぐために上げてください。 |
