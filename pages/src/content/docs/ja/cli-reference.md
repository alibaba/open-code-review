...
| `--delta-from <session-id>` | — | — | 変更の新しいバージョンをレビューし、diff が変わっていないファイルにはそのセッションの結果を再利用します。[変更の新しいバージョンをレビューする](#変更の新しいバージョンをレビューする)を参照してください。 |
...
レビュー指摘への対応、rebase、コミットの追加などで変更の新しいバージョンができたとき、
`--delta-from` は diff が変わったファイルだけをレビューし、残りは以前のセッションの結果を再利用します:

```bash
ocr review --from main --to feature-branch --delta-from <session-id>
```

ファイルが再利用されるのは、その diff が以前のセッションでレビューされた diff とバイト単位で
一致する場合だけなので、指摘と行番号は記録どおり有効です。両方のレビューで同じベースを使ってください。
ベースが動くとすべてのファイルの diff が変わります。入力が異なってよい点を除き、`--resume` と同じ規則に
従います: 範囲または単一 commit レビューのみ、リポジトリ・ルール・ファイルフィルタが同一であること、
provider や model の暗黙的な変更は不可。`--delta-from` は `--resume` や `--preview` と併用できません。

この実行は以前のセッションを親として記録し、再利用したファイルを `coverage.reused` に列挙し、
JSON の `resume` ブロックに `"delta": true` を出力します。再利用されたファイルは再レビューされないため、
変更されていないファイルの指摘のうち、変更されたファイルのコードに依存するものは再確認されません。
それが重要な場合はフルレビューを実行してください。
