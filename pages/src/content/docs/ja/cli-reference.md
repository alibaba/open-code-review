...
| コード | 意味 |
|---|---|
| `0` | レビューが完了しました。すべての選択項目が完了したか、未完了の項目が `--max-tokens-budget` による打ち切り（`failed(budget)` として報告）だけの場合です。 |
| `1` | 致命的エラー。引数の誤り、LLM エンドポイントを解決できない、すべてのファイルごとのサブエージェントが失敗した、などです。エラーテキストは stderr に出力されます。実行が*部分的*な場合（タイムアウトや provider エラーなど実際の失敗で 1 つでも項目が失敗し、他が完了していても）にも使われます。部分結果は非ゼロ終了の前に必ず公開されるため、JSON コンシューマは網羅状況の診断を失いません。 |
