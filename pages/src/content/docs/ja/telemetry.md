...
| `grouping.skipped` | 変更セットのファイル数が `GROUPING_MIN_FILES` を下回ったため、グルーピング呼び出しをスキップした。`strategy` は `bundle_all`（変更行数が `GROUPING_BUNDLE_LINE_THRESHOLD` 未満で、全ファイルを 1 グループに）または `per_file`（しきい値以上で、ファイルごとに 1 グループ）。 |
