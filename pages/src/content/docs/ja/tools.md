...
マッチは**空白に対して非依存**で、各スニペットはまずファイル内のままの形、次に
各行頭の `+`/`-` を 1 つ取り除いた形（diff からそのまま引用したスニペット向け）で
試します。原文どおりの形を先に試すことで、コード自体に属する行頭の `-`（YAML の
リスト項目など）を diff マーカーと取り違えません。マーカーを持たないスニペットは
どちらの形も同じになるため、1 回だけ試します。最後の手段として、
コメントは `start_line=0` で配信され、ユーザーに「問題は本物だが自分で位置を特定してほしい」と伝えます。
