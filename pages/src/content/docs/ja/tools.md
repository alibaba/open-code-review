...
1. **hunk の新側**——連続する **context + added** 行（deleted のみでも unchanged のみでもない）の一群で、
   新ファイルの行番号と `side=RIGHT` を得ます。失敗した場合、OCR は **hunk の旧側**——context +
   deleted 行——を再試行し、旧ファイルの行番号と `side=LEFT` を得ます。
...
解決された `side` は機械可読出力にも含まれます。`RIGHT` は変更後のファイル、`LEFT` は削除されたコードの変更前ファイルを示します。解決できないコメントでは省略されます。
