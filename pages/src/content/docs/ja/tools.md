...
マッチは**空白に対して非依存**で、各スニペットを 2 通りの読み方で順に試します：まず
ファイル内のままの形、次に各行頭の `+`/`-` を 1 つ取り除いた形（diff からそのまま引用
したスニペット向け）です。原文どおりの形を先に試すことで、コード自体に属する行頭の
`-`（YAML のリスト項目など）を diff マーカーと取り違えません。最後の手段として、
