...
### 要件や設計ドキュメントを使ってコードをレビューできますか？

`--background-file` でローカルの Markdown ファイルを共通のレビュー背景として渡せます。

```bash
ocr review --repo /path/to/repo --background-file docs/requirements.md
```

相対パスは選択したリポジトリのルートが基準です。内容は plan と main のレビュー
プロンプトに挿入されます。短い要件なら
`--background "Reject requests with an expired token"` も使えます。
両方を指定した場合はファイルが優先され、インラインのテキストは無視されます。

変更する動作と受け入れ条件に絞って記述してください。元のファイルの上限は
**1 MiB** です。サニタイズ後の内容が **2,000 文字**を超えると警告し、
**8,000 文字**を超えると拒否します（トークン数ではありません）。
社内ナレッジベースの内容は、必要な部分をローカルの Markdown に書き出してください。
このフラグはドキュメントの URL を取得しません。

特定のファイルに要件を関連付けるには、既存の[プロジェクトレビュールール](../review-rules/)
を使います。例えば、次を `<repo>/.opencodereview/rule.json` に保存します。
...
OCR は `<repo>/docs/requirements/auth.md` を読み、マッチするパスに対して
組み込みルールとともに適用します。ルールファイル参照は、空白を含まない単一行の
パスで、拡張子が `.md`、`.txt`、`.markdown` のものに対応し、上限は
**512 KiB** です。プロジェクトルールが参照するファイルはリポジトリ内に置きます。
最初にマッチしたエントリが優先されます。リポジトリのルートで
`ocr rules check src/auth/login.go` を実行すると、LLM を呼ばずに解決後のルールを確認できます。

この対応付けは明示的な設定です。OCR は `AGENTS.md` を自動的に読み込んだり、
各ファイルに適用する仕様書を自動で発見したりしません。背景情報は要件の確認に
役立ちますが、要件を網羅的に検証する保証ではありません。
