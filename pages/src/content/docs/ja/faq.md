...
### 一部のサブエージェントが失敗した場合、終了コードは？

OCR はファイルごとの失敗を隔離し、1 つの問題のあるファイルが 20 ファイルの
レビュー全体を巻き込まないようにします。部分結果も終了前に必ず公開されます。
ただし集計終了コードは、実行が不完全だった理由を区別します。すべての選択項目が
完了した場合（または不完全な項目が `--max-tokens-budget` による打ち切りで
`failed(budget)` と報告された場合のみ）`0`、タイムアウトや provider エラーなど
実際の失敗が 1 つでもあれば、他が全部成功していても非ゼロで終了します。これに
より CI は終了ステータスだけから不完全なレビューを検出できます。どのファイルが
失敗したかは、JSON モードの `warnings` 配列またはテキストモードの stderr を
確認してください。
