...
1. **行解決**（worker 内）: `existing_code` をスライディングウィンドウアルゴリズムで diff と照合し、正確な `start_line` / `end_line` と `side`（変更後は `RIGHT`、削除コードは変更前の `LEFT`）を計算します。照合に失敗した場合は両方ともデフォルトで `0` になり、`side` は省略されます。行範囲 `0` は「アンカーされていない」コメントの暗黙のシグナルであり、ユーザーが手動で位置を特定する必要があります（格納されるフラグはありません。下流のコンシューマーが `start_line == 0` をチェックします）。
