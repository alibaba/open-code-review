...
> **DNS リバインディング対策であり、アクセス制御ではありません。** ビューアは `Host` ヘッダーを
> ホワイトリスト（`localhost`、`127.0.0.1`、`::1`）と照合します。これは DNS リバインディングを
> 防ぎますが、リスナーに到達できるピアは `Host: localhost` を送信できます。ホワイトリストは認証ではなく、
> 接続できるネットワークピアを制限しません。具体的なバインドホスト（`--bind 192.168.1.10 -p 5483` など）は
> ホワイトリストに追加されます。**ワイルドカード**バインド（`--bind 0.0.0.0` または `--bind ::`）は
> 非ループバックホストを追加しません。ビューアを公開する際に特定の非ループバック `Host` ヘッダーを許可するには
