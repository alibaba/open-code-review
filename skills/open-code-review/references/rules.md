...
- **Extended Language Support**: Native rule resolution provides 44 built-in rulesets (covering Java, TS/JS, Go, Python, C/C++, Rust, Kotlin, PHP, Julia, Terraform, Bicep, Nix, Haskell, Nim, ArkTS, Astro, Protobuf, GraphQL, Prisma, FreeMarker, manifest files, etc.) and natively supports code reviews across 96 allowed file extensions including Swift, MATLAB, Objective-C (`.m`), R, Jupyter Notebook (`.ipynb`), Zig, Elm, Thrift, and Cap'n Proto.
...
- **Content-based sniffing for ambiguous extensions**: for `.m` files, OCR inspects first non-empty line heuristics to automatically distinguish between MATLAB and Objective-C, applying the appropriate ruleset.
