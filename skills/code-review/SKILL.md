...
Do not run `ocr review`, spawn review sub-agents, inspect progress events, or poll for completion. Do not terminate a running MCP call because it has produced no intermediate output; wait for the terminal result. If the host interrupts the call, use `ocr_review_wait` once to retrieve the existing execution's terminal result. Use `ocr_review_cancel` only for explicit cancellation. If the MCP tool is unavailable, report the integration error instead of falling back to the CLI.
