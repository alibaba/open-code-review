...
const { parseVersionOutput, shouldShowUpdateHint } = require("../scripts/version");
...
try {
  ...
  const installedVersion = parseVersionOutput(versionResult.stdout);
  if (hint.pkg && shouldShowUpdateHint(hint.version, installedVersion)) {
    console.error(
