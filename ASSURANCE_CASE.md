...
| **Separation of privilege** | Credential material is never compiled into the binary; optional persistence is limited to the user config file, which `saveConfig` keeps at mode `0600` (including re-chmod on existing files). The viewer's host guard is a distinct middleware layer. |
