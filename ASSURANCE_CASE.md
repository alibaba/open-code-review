...
3. Optional persistence in the user config file (`~/.opencodereview/config.json`) as `api_key` or `auth_token`. `ocr config set` and interactive provider setup write this file in plaintext; `saveConfig` (`cmd/opencodereview/provider_cmd.go`) creates it with mode `0600`, `chmod`s an existing file to `0600` before rewrite so sensitive bytes are not written while it remains broadly readable, and `chmod`s to `0600` again afterward as defense in depth (for example when tightening a prior `0644`).
...
| **Separation of privilege** | Credential material is never compiled into the binary; optional persistence is limited to the user config file, which `saveConfig` keeps at mode `0600` (pre-write chmod on existing files, plus post-write chmod). The viewer's host guard is a distinct middleware layer. |
