...
3. Optional persistence in the user config file (`~/.opencodereview/config.json`) as `api_key` or `auth_token`. `ocr config set` and interactive provider setup write this file in plaintext with mode `0600` (`saveConfig` in `cmd/opencodereview/provider_cmd.go`).
...
| **Separation of privilege** | Credential material is never compiled into the binary; optional persistence is limited to the user config file (mode `0600`). The viewer's host guard is a distinct middleware layer. |
...
| **A07:2021 Auth Failures** (CWE-798 Hard-coded Credentials) | API keys are never hardcoded in source. They may be read from environment variables, `api_key_cmd`/`auth_token_cmd`, or the user config file (`~/.opencodereview/config.json`, mode `0600`). Never logged; `ocr config set` masks them on display. | Mitigated |
