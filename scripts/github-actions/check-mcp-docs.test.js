...
const COMPLETE_LOCALES = ["en", "zh", "ja", "ru", "ko"];
...
const INVARIANTS = {
  ko: [
    [/`ocr scan`은 MCP를 로드하지 않습니다/, "MCP is review-only"],
    [/`tools`가 비어 있거나 없으면 \*\*도구 0개\*\*/, "empty allowlist exposes nothing"],
    [/모델 표시 범위와 실행 승인은 독립적인 두 단계/, "visibility and execution are independent"],
    [/거부, 취소, timeout, EOF[\s\S]*`tools\/call`을 보내지 않습니다/, "failed approval never calls the tool"],
    [/`allow`[\s\S]*allowlist를 넓히지 않습니다/, "allow cannot widen capabilities"],
    [/annotation[\s\S]*자동 승인에 사용하지 않습니다/, "annotations never authorize"],
    [/검색은[\s\S]*`tools\/list`[\s\S]*실행이나 모델 등록은 하지 않습니다/, "discovery is non-executing"],
    [/mcp__<server-slug>__<tool-slug>__<16-hex>[\s\S]*충돌[\s\S]*부분 노출을 막습니다/, "names and collisions fail closed"],
    [/전체 환경을 상속하지 않습니다[\s\S]*마스킹[\s\S]*교차 출처 redirect[\s\S]*거부/, "credentials and redirects are constrained"],
  ],
  en: [
...
for (const supportPage of SUPPORT_PAGES) {
  ...
  assert.ok(korean.includes("../mcp/"), `ko/${supportPage} must link to the Korean MCP guide`);
  assert.doesNotMatch(korean, /영문 MCP 가이드|한국어 전용 MCP 페이지는 없/, `ko/${supportPage} must not claim the Korean page is missing`);
}
...
console.log("MCP documentation contracts passed for en, zh, ja, ru, and ko.");
