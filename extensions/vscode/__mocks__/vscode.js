...
module.exports = {
  ...
  workspace: { workspaceFolders: [{ uri: { fsPath: '/mock' } }], applyEdit: () => Promise.resolve(true), openTextDocument: () => Promise.resolve({ lineCount: 10, lineAt: () => ({ text: '' }) }) },
  window: { showWorkspaceFolderPick: () => Promise.resolve(undefined), showTextDocument: () => Promise.resolve(), showErrorMessage: () => {}, showWarningMessage: () => Promise.resolve(undefined), createOutputChannel: () => ({ appendLine: () => {}, dispose: () => {} }), createWebviewPanel: () => ({ webview: { html: '', onDidReceiveMessage: () => ({ dispose: () => {} }), asWebviewUri: (u) => u }, onDidDispose: () => ({ dispose: () => {} }), reveal: () => {}, dispose: () => {} }) },
  ...
  comments: { createCommentController: () => ({ createCommentThread: (uri, range, comments) => ({ uri, range, comments, dispose: () => {} }), dispose: () => {} }) },
