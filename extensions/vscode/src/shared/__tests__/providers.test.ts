...
import { isPresetProvider, lookupPreset, mergeModelLists, PROVIDER_PRESETS } from '../providers';
...
describe('generated provider presets', () => {
  ...
  it.each(generatedPresets)('preserves metadata, model order and the default for $name', (preset) => {
    ...
    const models = mergeModelLists(found?.models ?? [], ['user-model', preset.models[0]]);
