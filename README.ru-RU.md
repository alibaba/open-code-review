...
**1.1 Необязательные псевдонимы команд**

Часто используемые команды можно сократить.
...
После этого `ocr cp` и `ocr rq` работают так же, как полные команды. Список
псевдонимов можно посмотреть через `ocr alias list`, а удалить — через
`ocr alias rm <name>`.
