...
func TestDismissListNoPanicWhenStorePathUnresolvable(t *testing.T) {
	// Regression: LoadDismissals returns (nil, err) when the home directory
	// is unresolvable; runDismissList must return an error, not panic on a
	// nil store deref. Every os.UserHomeDir source is cleared: on Windows the
	// lookup falls back to USERPROFILE (and HOMEDRIVE+HOMEPATH) when HOME is
	// unset, which would otherwise resolve a path and hide the regression.
	t.Setenv("HOME", "")
