...
func outputSARIF(comments []model.LlmComment, version string, warnings []agent.AgentWarning, manifest *session.RunManifest) error {
	return outputSARIFTo(os.Stdout, comments, version, warnings, manifest)
}

func outputSARIFTo(outWriter io.Writer, comments []model.LlmComment, version string, warnings []agent.AgentWarning, manifest *session.RunManifest) error {
	report := sarifReport{
		Schema:  sarifSchema,
