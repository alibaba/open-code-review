...
func executeReviewContext(ctx context.Context, opts reviewOptions) (retErr error) {
	...
	defer closeReviewMCPClients(mcpRuntime.clients)
	...
	rawHolder := rt.RawHolder
	if rawHolder != nil && len(mcpRuntime.toolDefs) > 0 {
		fmt.Fprintln(os.Stderr, "[ocr] Raw logging disabled for this review: MCP tools may carry sensitive arguments.")
		rawHolder = nil
	}
	closeRaw := bindRawWriter(rawHolder, cc.RepoDir, ag.Session())
	defer closeRaw()
