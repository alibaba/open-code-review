...
// admissionE2EChangeLines is the per-file changed-line count. Two thresholds
// ride on it: it must exceed the plan-phase skip threshold (5 lines), or the
// run skips planning and the e2e silently loses a path; and the three files'
// combined churn must reach GROUPING_BUNDLE_LINE_THRESHOLD (200), or grouping
// bundles the change set into one group and the baseline loses the parallel
// dispatch it exists to measure.
const admissionE2EChangeLines = 80
