...
func emitRunResult(
	...
	return emitRunResultTo(ctx, ag, comments, startTime, outputFormat, audience, q, llmIdentity, os.Stdout, retryReport)
}
...
func emitRunResultTo(
	...
	if machineReadable && manifest == nil && len(comments) == 0 && ag.FilesReviewed() == 0 {
		...
		return outputSARIFTo(outWriter, nil, Version, ag.Warnings(), manifest)
	}
	...
	if outputFormat == "json" {
		...
		return outputJSONWithWarningsAndDiagnosticsTo(outWriter, comments, ag.Warnings(), ag.FilesReviewed(),
			...
			usageStatus, unknownRequests, retryReport)
	}
	if outputFormat == "sarif" {
		return outputSARIFTo(outWriter, comments, Version, ag.Warnings(), manifest)
	}
