...
func outputPreviewText(p *agent.DiffPreview, out io.Writer) {
	...
	if p.ExcludedCount > 0 {
		...
		// Provider-directory files can number in the thousands and no rule can make
		// them reviewable, so they collapse into one line instead of a row each.
		...
		for _, e := range p.Entries {
			if e.ExcludeReason == agent.ExcludeProviderDirectory {
				// The provider matches its directories from the repository root, so
				// the first path component is the directory that matched.
				dir, _, _ := strings.Cut(e.Path, "/")
