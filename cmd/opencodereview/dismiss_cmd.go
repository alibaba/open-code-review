...
// dismissCmd surfaces the dismiss dispatcher on the cobra root. Flag parsing
// stays inside runDismiss (std flag package), so cobra passes everything
// through untouched.
var dismissCmd = &cobra.Command{
	Use:   "dismiss",
	Short: "Record and manage dismissed findings (suppress from review)",
	Long: "Record and manage dismissed findings, stored per repo under ~/.opencodereview/dismissals/.\n" +
		"Dismissed findings are suppressed from ocr review output.\n\n" +
		"Subcommands:\n" +
		"  add <session-id> <finding-ref>   Dismiss a finding from a session\n" +
		"  list                             List dismissed findings for this repo\n" +
...
// dismissFlags is the parsed flag set shared by the dismiss subcommands:
// --repo plus the remaining positional arguments. -h/--help is detected
// during parse and reported via showHelp.
type dismissFlags struct {
	repo     string
	rest     []string
	showHelp bool
}

// parseDismissFlags parses args with the standard flag package, treating
// -h/--help as a help request rather than an error.
func parseDismissFlags(name string, args []string) (*dismissFlags, error) {
	f := &dismissFlags{}
	fs := flag.NewFlagSet(name, flag.ContinueOnError)
	fs.SetOutput(io.Discard)
	fs.StringVar(&f.repo, "repo", "", "root directory of the git repository (default: current dir)")
...
func runDismissAdd(args []string) error {
	a, err := parseDismissFlags("ocr dismiss add", args)
	if err != nil {
		return err
	}
	if a.showHelp {
...
func runDismissList(args []string) error {
	a, err := parseDismissFlags("ocr dismiss list", args)
	if err != nil {
		return err
	}
	if a.showHelp {
...
func runDismissRemove(args []string) error {
	a, err := parseDismissFlags("ocr dismiss remove", args)
	if err != nil {
		return err
	}
	if a.showHelp {
