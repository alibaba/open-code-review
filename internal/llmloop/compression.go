...
func (r *Runner) runCompression(ctx context.Context, msgs []llm.Message, filePath string, persistCancellationOpt ...bool) ([]llm.Message, error) {
	persistCancellation := len(persistCancellationOpt) > 0 && persistCancellationOpt[0]
	...
	resp, err := r.client.CompletionsWithCtx(reqCtx, llm.ChatRequest{
		Model:     r.deps.Model,
