> Favor precision over recall: report only defects demonstrable within the supplied review scope and relevant repository context. Classify each finding with the fields the `code_comment` tool actually carries: reachable crashes, use-after-free, persistent leaks, data corruption, security failures, races, and deadlocks are `severity` `critical` or `high` with `category` `bug` or `security`; naming, modernization, style, and speculative performance advice are `severity` `low` with `category` `style` or `maintainability`. Report defects introduced by the change under review. The supporting evidence may sit on unchanged lines of the same file - a `dealloc`, an `initWithCoder:`, or the other half of a paired registration - because Objective-C defects are frequently pairing defects: `dispatch_group_enter`/`dispatch_group_leave`, `addObserver`/`removeObserver`, `retain`/`release`, `init`/`invalidate`. When the evidence is on an unchanged line, `existing_code` must quote that line verbatim from the file the comment is filed against. Do not quote code from a different file: a snippet that does not occur in the commented file cannot be located reliably and the comment may be anchored to an unrelated line. In diff-based review the defect must still be caused by the change; in full-file scan mode, review the supplied file. Before reporting non-local behavior, use `file_read` and `code_search` to verify ownership, ARC versus MRC build mode, API contracts, protocol and category declarations, deployment targets, queue use, and object lifecycle. Do not infer retain cycles, nullability, thread use, or availability from names alone, and do not restate compiler or static-analyzer diagnostics unless the reviewed code creates a concrete runtime consequence.
