...
- Do not report unused imports, parameters, or assignments in `.pyi` stub files; a stub declares an interface it never executes, so re-exported imports and unreferenced parameter names are expected rather than dead
