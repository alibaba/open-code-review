...
- **Staged changes**: when the user requests staged changes or the staging area, use `--staged` to review a frozen index snapshot against the captured `HEAD`. It cannot be combined with `--from`, `--to`, `--commit`, or `--resume`.
...
- **Staged preview**: use `--staged --preview`, optionally with `--format json`, to inspect staged files, exclusions, and snapshot identity. Each invocation captures its own snapshot.
...
| "review staged changes" / "review the staging area" | `ocr review --audience agent -b "context" --staged` |
...
| "what staged changes would be reviewed?" | `ocr review --staged --preview --format json` |
...
- **Untracked files are reviewed in workspace mode** — running bare `ocr review` includes staged, unstaged, *and* untracked changes. Use `--staged` for a review of the staging area.
...
- **Resume an interrupted review** — a failed or interrupted range/commit review can be continued with `ocr review --resume <id>` using the same `--from`/`--to` or `--commit` target (the id is printed as `retry with: --resume <id>` on failure, or find it with `ocr session list`). Workspace and staged sessions cannot be resumed.
...
The installed CLI does not support staged snapshot review. Report the unsupported flag and retain the user's requested staged scope. Continue when a CLI build supporting `--staged` is available, using the same staged command. Do not retry as a workspace review.
